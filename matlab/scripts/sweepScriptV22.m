% =========================================================================
% Final Synthesized H-Matrix Parameter Sweep Script v18.0
%
% Description:
% This script tests the hypothesis that extreme signal diversity can overcome
% geometric correlations. It reverts to a regular imaging grid but introduces
% a much wider range of random time delays, in addition to per-element
% frequency and phase randomization.
%
% v18.0 Features:
% - Returns to a regular, structured imaging grid to avoid packing errors.
% - Introduces a sweep parameter for "aggressive" time delays (up to 200us).
% - Retains maximum transmitter, frequency, and phase randomization.
% =========================================================================
clear; clc; close all;
...
base_config.grid_step_m = 0.02; % Temporarily reduced for fast testing
base_config.num_acquisitions = 5;
...
function H = generate_h_matrix_final(config)
    ...
    xdc_free(RxAperture);
    field_end;
    % Print H matrix stats for debugging
    if nnz(H) > 0
