% =========================================================================
% Advanced Comparative H-Matrix Diagnostic Script v3.4
%
% Description:
% This version tests the hypothesis that random PHASE DIVERSITY will
% create a more incoherent H matrix. It builds and compares four matrices,
% with the final one adding random phase shifts to each transmitted pulse.
% =========================================================================
...
H_apodized = generate_h_matrix(num_acquisitions, apodized_delays_us, apodized_frequencies_hz, c, fs, ...
    pmut_width_m, pmut_spacing_m, kerf_m, grid_width_m, grid_depth_start_m, grid_depth_end_m, grid_step_m, ...
    excitation_amplitude, 'apodization');

disp('--- Building H_phased Matrix (random delays, freqs, & phases) ---');
rng('default');
phased_delays_us = sim_delays_us;
phased_frequencies_hz = sim_frequencies_hz;
H_phased = generate_h_matrix(num_acquisitions, phased_delays_us, phased_frequencies_hz, c, fs, ...
    pmut_width_m, pmut_spacing_m, kerf_m, grid_width_m, grid_depth_start_m, grid_depth_end_m, grid_step_m, ...
    excitation_amplitude, 'phase');
...
run_diagnostics('Phased', H_phased, fs, num_acquisitions, imageResolution, x_coords_img, z_coords_img, output_folder);

disp('--- All diagnostics complete! ---');

%% --- Helper Function to Generate an H Matrix ---
function H = generate_h_matrix(num_acquisitions, delays_us, frequencies_hz, c, fs, pmut_width_m, pmut_spacing_m, kerf_m, grid_width_m, grid_depth_start_m, grid_depth_end_m, grid_step_m, excitation_amplitude, diversity_mode)
    field_init(-1);
    set_field('fs', fs);
    set_field('c', c);
    ...
    for r_acq = 1:num_acquisitions
        ...
        % --- NEW: Generate random phase shifts if in 'phase' mode ---
        if strcmp(diversity_mode, 'phase')
            phase_shifts = 2 * pi * rand(1, num_tx_active);
        else
            phase_shifts = zeros(1, num_tx_active);
        end

        for i = 1:num_tx_active
            tx_durations = cycles / current_freqs(i);
            t_signal = 0 : 1/fs : tx_durations;
            signal_base = sin(2 * pi * current_freqs(i) * t_signal + phase_shifts(i)); % Add phase shift here
