% =========================================================================
% Comprehensive H-Matrix Parameter Sweep Script v6.7 (Definitive Fix)
%
% Description:
...
%
% v6.7 Fixes & Features:
% - Reverted to the robust v6.0 simulation model which produced the highest
%   number of non-zero columns.
% - DEFINITIVE FIX: Drastically increased excitation_amplitude to overcome
%   acoustic attenuation in air, which was the root cause of the weak
%   signal and near-zero column norms.
% - Updated diagnostics to report max and mean coherence in scientific
...
base_config.excitation_amplitude = 1e12; % DRAMATICALLY INCREASED to overcome attenuation
...
function H = generate_h_matrix(config)
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % *** DEFINITIVE FIX: Apply impulse AND correct excitation ***
...
function results = run_diagnostics(name, H, config, output_folder)
    ...
    % --- NEW: Report with scientific notation ---
    fprintf('  Max Mutual Coherence: %.4e\n', max_coherence);
    fprintf('  Mean Mutual Coherence: %.4e\n', mean_coherence);
    fprintf(logfid, 'Max Mutual Coherence: %.4e\n', max_coherence);
    fprintf(logfid, 'Mean Mutual Coherence: %.4e\n', mean_coherence);
