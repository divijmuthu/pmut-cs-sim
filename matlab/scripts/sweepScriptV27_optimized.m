%% sweepScriptV27_optimized.m - Optimized H-Matrix Optimization
% Based on V25_lightweight's successful approach with targeted optimizations
% Key insights: Time-domain diversity works, focus on parameter tuning

clear; clc; close all;

%% Base Configuration (Optimized from V25_lightweight results)
base_config = struct();
...
base_config.num_acquisitions = 20;      % Fast execution (like V23)
...
%% Optimized Parameter Sweep (Based on V25_lightweight insights)
p = struct();

% Best performers from V25_lightweight: 5 transmitters, 500μs delays, uniform apodization
p.num_active_tx_sweep = {5};            % Best from V25_lightweight
p.delay_rand_sweep_us = {500};          % Best from V25_lightweight
p.apodization_sweep = {'uniform'};      % Best from V25_lightweight

% Focus on the most promising variations from V25_lightweight
p.grid_step_sweep = {0.008, 0.010};     % Best grid densities
p.tx_pool_width_sweep = {0.200, 0.250}; % Best transmitter spacings
p.frequency_bandwidth_sweep = {[45000, 65000], [40000, 70000]}; % Best frequency ranges

%% Output Configuration
timestamp = datestr(now, 'mmddyy_HHMMSS');
output_folder = fullfile('sweep_output_optimized_v27', timestamp);
if ~exist(output_folder, 'dir')
    mkdir(output_folder);
end

fprintf('=== Optimized H-Matrix Optimization V27 ===\n');
fprintf('Based on V25_lightweight successful approach\n');
...
fprintf('Total parameter combinations: %d (optimized for speed)\n\n', num_combinations);
...
for i = 1:num_combinations
    ...
    fprintf('Starting H matrix generation (V25_lightweight approach)...\n');
    tic;
    
    % Generate H matrix using V25_lightweight's successful approach
    H = generate_h_matrix_v25_style(config);
    
    generation_time = toc;
    fprintf('H matrix generation completed in %.2f seconds\n', generation_time);
    
    % Run diagnostics (same as V25_lightweight)
    fprintf('Running diagnostics...\n');
    metrics = run_optimized_diagnostics(H, config);
    ...
    fprintf('Max Coherence: %.4f (target: <0.9)\n', metrics.max_coherence);
    ...
    if metrics.condition_number < 200
...
generate_optimized_summary_report(results, output_folder);

%% Save Complete Results (LIGHTWEIGHT)
save(fullfile(output_folder, 'complete_results.mat'), 'results', 'base_config', 'p');

fprintf('=== Optimized Optimization Complete! ===\n');
fprintf('Results saved to: %s\n', output_folder);
fprintf('Total disk usage: ~%d MB\n', round(num_combinations * 0.05));

%% Helper Functions

function H = generate_h_matrix_v25_style(config)
    % Initialize Field II
    fprintf('  Initializing Field II...\n');
    field_init(-1);
    
    % Setup Field II transducers (using 2D array approach like V25_lightweight)
    fs = config.fs;
    c = config.c;
    vgrid_N = 10;
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % Generate diverse excitation signals (like V25_lightweight)
        ...
        % Generate composite waveform (like V25_lightweight)
    ...
    % Assemble H matrix using V25_lightweight's time-domain approach
...
function h_aligned = interpolate_h_response(t_current, h_current, t_common)
    % Interpolate H response to common time axis (from V25_lightweight)
...
function metrics = run_optimized_diagnostics(H, config)
    % Optimized diagnostics (same as V25_lightweight)
    metrics = struct();
    ...
    % Coherence analysis (same as V25_lightweight)
    ...
    % Advanced metrics (same as V25_lightweight)
    if size(Hn, 2) > 1
        metrics.condition_number = cond(full(Hn));
        
        % RIP proxy (same as V25_lightweight)
        K = min(10, size(Hn, 2));
...
function generate_optimized_summary_report(results, output_folder)
    % Create summary CSV
    fprintf('Creating optimized summary CSV...\n');
    summary_data = [];
