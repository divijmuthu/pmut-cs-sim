% =========================================================================
% sweepScriptV23.m -- Enhanced H-matrix sweep with robust debugging and denser H
% Based on sweepScriptV22.m (0.66–0.82 max coherence, fixed grid, 20mm pMUTs)
% =========================================================================
clear; clc; close all;
...
base_config.c = 343; % In-air
base_config.fs = 1e6;
base_config.pmut_width_m = 0.020; % Actual transducer size (2 mm)
base_config.tx_pool_width_m = 0.200; % 10x10 grid, 20 mm pitch
base_config.grid_width_m = 0.150;
base_config.target_distance_m = 0.150;
base_config.grid_depth_range_m = 0.10;
base_config.grid_step_m = 0.01; % Denser grid for more realistic H
base_config.num_acquisitions = 20; % More acquisitions for denser H
...
p.num_active_tx_sweep = {3, 5}; % Only 3 and 5
p.delay_rand_sweep_us = {50, 500}; % Up to 500 us
p.apodization_sweep = {'uniform', 'random'};

%% --- 2. SETUP OUTPUT & RUN SWEEP ---
disp('--- Initializing Final H-Matrix Sweep with Aggressive Signal Diversity ---');
session_timestamp = datestr(now, 'mmddyy_HHMMSS');
base_output_dir = fullfile(pwd, 'sweep_output_aggressive_signal_div');
...
function H = generate_h_matrix_final(config)
    % Always initialize Field II first!
    field_init(-1);
    fs = config.fs;
    c = config.c;
    vgrid_N = 10;
    ...
    % --- Fixed Imaging Grid (regular meshgrid) ---
    ...
    xdc_free(RxAperture);
    field_end;
    % Enhanced debugging after H assembly
    if nnz(H) > 0
        ...
        num_nz_cols = sum(col_norms > 1e-12);
        nz_col_indices = find(col_norms > 1e-12);
        fprintf('Number of nonzero columns: %d (out of %d)\n', full(num_nz_cols), full(size(H,2)));
        fprintf('Indices of nonzero columns: ');
        disp(nz_col_indices);
        persistent run_counter;
        if isempty(run_counter), run_counter = 1; else, run_counter = run_counter + 1; end
        if run_counter == 1
            fprintf('Column norms for first run: ');
            disp(col_norms);
        end
        if num_nz_cols == 0
            warning('All columns are zero!');
        elseif num_nz_cols < 0.5*size(H,2)
            warning('More than half the columns are zero!');
        end
    else
        fprintf('H matrix stats: all zeros, nnz=0\n');
        warning('All entries in H are zero!');
    end
end
...
function results = run_diagnostics(name, H, config, output_folder)
    ...
    close(fig_coh_hist);
    % --- SVD/Condition Number Diagnostics ---
    fprintf(logfid, 'SVD/Condition Number Diagnostics:\n');
    num_sv_cols = min(1000, size(Hn,2));
    if num_sv_cols > 1
        H_subset = full(Hn(:, randperm(size(Hn,2), num_sv_cols)));
        S = svd(H_subset, 'econ');
        fig_svd = figure('Name', ['SVD Decay: ' name], 'visible', 'off');
        semilogy(S, 'LineWidth', 1.5);
        title('Singular Value Decay of H Matrix');
        xlabel('Singular Value Index'); ylabel('Magnitude'); grid on;
        saveas(fig_svd, fullfile(output_folder, ['diag_' lower(name) '_svd_decay.png']));
        close(fig_svd);
