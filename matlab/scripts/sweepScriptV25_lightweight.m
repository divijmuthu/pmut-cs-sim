%% sweepScriptV25_lightweight.m - Lightweight H-Matrix Optimization
% Only saves diagnostic images and summary data - NO massive H matrices!
...
output_folder = fullfile('sweep_output_lightweight_v25', timestamp);
if ~exist(output_folder, 'dir')
    mkdir(output_folder);
end

fprintf('--- Lightweight H-Matrix Optimization V25 ---\n');
...
for i = 1:num_combinations
    ...
    % Store results (LIGHTWEIGHT - no H matrix!)
    results.configs{i} = config;
    results.metrics{i} = metrics;
    results.scenes{i} = scene;
    % NO results.H_matrices{i} = H; - this was the data bloat!
    
    % Save individual run results (LIGHTWEIGHT - only metrics and config!)
    run_folder = fullfile(output_folder, run_name);
    if ~exist(run_folder, 'dir')
        mkdir(run_folder);
    end
    
    % Save lightweight results (NO H matrix!)
    save(fullfile(run_folder, 'results.mat'), 'scene', 'config', 'metrics', 'acquisition_info');
    % NO 'H' in the save - this was the main data bloat!
    ...
    % Clear H matrix from memory to save RAM
    clear H;
end
...
%% Save Complete Results (LIGHTWEIGHT - no H matrices!)
save(fullfile(output_folder, 'complete_results.mat'), 'results', 'base_config', 'p', 'analytics_config');
% NO 'results' contains H matrices anymore!

fprintf('--- Lightweight Optimization Complete! ---\n');
fprintf('Results saved to: %s\n', output_folder);
fprintf('Total disk usage: ~%d MB (vs ~5GB in original)\n', ...
    round(num_combinations * 0.1)); % Rough estimate
