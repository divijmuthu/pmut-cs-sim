% =========================================================================
% Final Synthesized H-Matrix Parameter Sweep Script v11.0
%
% Description:
% This version introduces a powerful form of geometric diversity by
% randomizing the pool of possible transducer locations for each run.
% This breaks the systematic geometric correlations that dominated
% previous results.
%
% v11.0 Final Features:
% - Implements a sparse, randomized grid of potential transducer locations.
% - Keeps the receiver fixed at the center [0,0,0].
% - Retains per-element frequency and phase randomization for the signal.
% =========================================================================
clear; clc; close all;
...
% --- NEW: Define the area for the random transducer pool ---
base_config.tx_pool_width_m = 0.200; % 200mm area for locations
...
disp('--- Initializing Final H-Matrix Sweep with Randomized TX Positions ---');
session_timestamp = datestr(now, 'mmddyy_HHMMSS');
base_output_dir = fullfile(pwd, 'sweep_output_random_tx_grid');
...
function H = generate_h_matrix_final(config)
    ...
    % --- NEW: Define a SPARSE, RANDOMIZED pool of potential element locations ---
    total_elements = 81; % Keep the same number of potential sites
    half_width = config.tx_pool_width_m / 2;
    x_positions = (rand(total_elements, 1) - 0.5) * 2 * half_width;
    y_positions = (rand(total_elements, 1) - 0.5) * 2 * half_width;
    physical_element_centers = [x_positions, y_positions, zeros(total_elements, 1)];

    % --- Define the FIXED Receiver at the center ---
    RxAperture = xdc_focused_array(1, config.pmut_width_m, config.pmut_width_m, 0, 1, 1, [0 0 0], 1, 1, [0 0 0]);
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        active_centers = physical_element_centers(active_indices, :);
        TxAperture = xdc_focused_array(num_active_tx, config.pmut_width_m, config.pmut_width_m, 0, 1, 1, active_centers, 1, 1, [0 0 0]);
