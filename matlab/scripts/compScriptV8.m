% =========================================================================
% Comprehensive H-Matrix Parameter Sweep Script v5.3
%
% Description:
...
%
% v5.3 Fixes:
% - Aligned all core physical parameters (speed of sound, transducer
%   geometry, imaging grid location, acquisition count, frequency range)
%   with the user-provided baseline simulation.
% - Corrected the simulation physics to produce non-zero, meaningful results
%   for an in-air simulation (c = 343 m/s).
% =========================================================================
...
base_config.grid_depth_range_m = 0.05; % Look +/- 2.5cm around target
base_config.grid_step_m = 0.004;
base_config.num_acquisitions = 150; % Number of acquisitions
...
p.delay_rand_sweep_us = {0, 12, 50}; % Added 50us to match baseline max
...
param_grid = allcomb(p.num_active_tx_sweep, p.waveform_sweep, ...
                     p.delay_rand_sweep_us, p.apodization_sweep);
...
for i = 1:num_total_runs
    ...
    run_name = sprintf('run%03d_tx%d_%s_del%d_%s', i, ...
        config.num_active_tx, config.waveform_type, ...
        config.max_delay_rand_us, config.apodization_mode);
...
function H = generate_h_matrix(config)
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % 1. Apodization: Turn on only the active elements
        ...
        % 2. Excitation Waveform (a single waveform for the whole aperture)
        % This aligns with the baseline's simplified model.
        mean_freq = (config.f_min_hz + config.f_max_hz) / 2;
        cycles = 3;
        duration = cycles / mean_freq;
        t = 0 : 1/fs : duration;
        signal_base = sin(2 * pi * mean_freq * t);
        
        switch config.waveform_type
            case 'tukey_sine',    window = tukeywin(length(t), 0.25)';
            case 'gaussian_sine', window = gausswin(length(t), 2.5)';
        end
        waveform_data = signal_base .* window * 1e8;
        xdc_impulse(TxAperture, waveform_data);
        xdc_excitation(TxAperture, 1); % Use a single excitation for all elements
