% =========================================================================
% UNIFIED PMUT ULTRASOUND SIMULATION SCRIPT (v2.0 - Real Data Reconstruction)
%
% Description:
...
% ADC measurements) and uses them to perform a reconstruction. The H-matrix
% is simulated using the exact parameters from the experiment, and the
% reconstruction is performed on the real ADC data.
%
% =========================================================================
clear; clc; close all;
...
% Use the latter 150 acquisitions as requested
...
% NOTE: h5py saves data in (samples, acquisitions), so we slice the 2nd dim
...
% The H-matrix is assembled by stacking acquisitions. We must do the same
% for the measurement vector 'b'. The transpose and reshape (:) operation
% stacks the columns of the ADC data matrix into one long vector.
b_vector = double(adc_data(:));

%% ===== 5. RECONSTRUCT IMAGE =====
fprintf('\n--- Step 4: Reconstructing Image via ADMM ---\n');
tic;
% Note: We pass an empty scene_matrix because we don't have the ground truth.
...
function [H, imaging_grid] = generate_h_matrix(config, delays_us, frequencies_hz)
    ...
    for r_acq = 1:config.num_acquisitions
        % --- Use EXPERIMENTAL profiles ---
        current_freq = frequencies_hz(r_acq);
        current_delays_us = delays_us(r_acq, :);
        
        % The experimental data used one frequency for all 3 pMUTs
    ...
    % --- Assemble H to match the fixed-size blocks of real data ---
    H = zeros(config.TOTAL_SAMPLES * config.num_acquisitions, N_pixels);
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % Ensure we don't write past the allocated block for this acquisition
...
% =========================================================================
% SCENE AND SIMULATION (Only used for plotting ground truth in sim mode)
% =========================================================================
function scene_matrix = create_scene(imaging_grid, params)
    ...
    if length(params.target_sizes_mm) < num_targets, params.target_sizes_mm = repmat(params.target_sizes_mm, 1, ceil(num_targets/length(params.target_sizes_mm))); end
    if length(params.target_intensities) < num_targets, params.target_intensities = repmat(params.target_intensities, 1, ceil(num_targets/length(params.target_intensities))); end
    ...
    for i = 1:num_targets
        ...
        for dz = -target_radius_pixels:target_radius_pixels, for dx = -target_radius_pixels:target_radius_pixels
            ix_target = ix_scene + dx; iz_target = iz_scene + dz;
