% =========================================================================
% Final Synthesized H-Matrix Parameter Sweep Script v15.1
%
% Description:
% This definitive version achieves full geometric diversity using a robust
% "virtual grid" method. A single, large grid of potential transducer
% locations is defined. For each acquisition, a random subset of these
% elements is activated, creating a unique, sparse, and random transmit
% aperture for every shot. This method avoids all previous syntax errors.
%
% v15.1 Fixes:
% - Corrected physical sizing error by reducing the virtual grid density
%   (vgrid_N) to a value that fits within the defined random area.
% =========================================================================
clear; clc; close all;
...
% --- Define the area for the random transducer pool ---
...
disp('--- Initializing Final H-Matrix Sweep with Virtual Grid Randomization ---');
session_timestamp = datestr(now, 'mmddyy_HHMMSS');
base_output_dir = fullfile(pwd, 'sweep_output_virtual_grid_tx');
...
function H = generate_h_matrix_final(config)
    ...
    % *** FIX IS HERE: Reduced grid density to be physically possible ***
    vgrid_N = 10; % A 10x10 virtual grid gives 100 possible locations
    vgrid_total_elements = vgrid_N * vgrid_N;
    
    % Calculate pitch and kerf to make the grid span the desired random area
    vgrid_pitch = config.tx_pool_width_m / (vgrid_N - 1);
    vgrid_kerf = vgrid_pitch - config.pmut_width_m;
    if vgrid_kerf < 0.0001 % Ensure a minimum physical gap
        error('pMUT width is too large for the virtual grid. Increase tx_pool_width_m or decrease vgrid_N/pmut_width_m.');
    end

    % --- Define the full virtual transmit aperture ONCE ---
    % All 100 elements are defined here, but we will activate subsets later.
    ...
    for r_acq = 1:config.num_acquisitions
        % --- Randomly select which elements from the virtual grid are active ---
        active_indices = randperm(vgrid_total_elements, num_active_tx);
        
        % --- Generate COMPOSITE WAVEFORM for the active elements ---
        ...
        % --- Activate ONLY the selected elements using apodization and excitation ---
        ...
        % --- Apply delays ONLY to the selected elements ---
