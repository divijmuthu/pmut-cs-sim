% =========================================================================
% Comprehensive H-Matrix Parameter Sweep Script v6.6 (Definitive Fix)
%
% Description:
...
%
% v6.6 Fixes & Features:
% - Reinstated TRUE per-element frequency diversity, the most physically
%   accurate model. A unique waveform is now generated for each active
%   element in each acquisition and assigned correctly with ele_waveform.
% - This definitive model combines the robust temporary aperture logic with
%   the correct excitation method to produce a strong, valid acoustic field.
% - Updated diagnostics to report max coherence in scientific notation and
%   to calculate and report the mean coherence.
% =========================================================================
...
function H = generate_h_matrix(config)
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % Excitation: Generate and assign a unique waveform for each active element
        frequencies_hz = config.f_center_hz + (rand(1, num_actually_active) - 0.5) * 2 * config.max_freq_rand_hz;
        for i = 1:num_actually_active
