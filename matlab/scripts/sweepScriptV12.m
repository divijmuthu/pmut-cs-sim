% =========================================================================
% Final Synthesized H-Matrix Parameter Sweep Script v9.1
%
% Description:
% This definitive script integrates a sophisticated waveform generation
% model based on pMUT physics into the robust sweeping framework. For each
% acquisition, every active transducer is assigned its own unique, random
% frequency from a 45-65 kHz band. These signals are combined into a
% single composite waveform to maximize the diversity of the transmitted
% acoustic field.
%
% v9.1 Final Features:
% - Uses a pMUT-specific waveform: 3-cycle, Tukey-windowed sine wave.
% - Implements per-element frequency diversity from a fixed 45-65 kHz range.
% - Sweep is focused on the most impactful parameters:
...
function H = generate_h_matrix_final(config)
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % --- Generate a COMPOSITE WAVEFORM with per-element frequency diversity ---
        ...
        for k = 1:num_active_tx
            ...
            window = tukeywin(length(t), 0.25)'; % Use Tukey window as specified
