%% ADMM Parameter Sweep and Diagnostics
% Systematic testing of ADMM parameters and H matrix diagnostics
...
%% ===== CONFIGURATION =====
fprintf('=== ADMM PARAMETER SWEEP AND DIAGNOSTICS ===\n');

% Create output directory
timestamp = datestr(now, 'mmddyy_HHMMSS');
output_folder = sprintf('admm_sweep_output/%s', timestamp);
if ~exist(output_folder, 'dir')
    mkdir(output_folder);
end
fprintf('Saving results to: %s\n', output_folder);

% Load parameters from merged script
params = struct();
params.pmut_resonance_freq = 57700;  % Hz
params.c = 343;  % m/s
params.fs = 100e6;  % 100 MHz sampling
params.grid_width_m = 0.1;
params.grid_depth_range_m = 0.1;
params.grid_step_m = 0.0025;  % 2.5mm grid step
params.target_distance_m = 0.05;  % 5cm target distance
params.num_acquisitions = 20;
params.num_active_tx = 5;
params.tx_pool_width_m = 0.08;
params.pmut_width_m = 0.002;
params.frequency_offset_hz = 0;
params.pmut_bandwidth = 1000;
params.impulse_duration_us = 10;
params.excitation_amplitude = 1.0;
params.apodization_mode = 'random';
params.max_delay_rand_us = 5;
params.rho_admm = 1.0;
params.admm_max_iter = 100;
params.admm_tol = 1e-6;

% Test configuration (use realistic for good targets)
config = struct();
config.description = 'Realistic Challenge: 5mm targets, 20mm spacing';
config.target_size_mm = 5.0;
config.grid_spacing_mm = 20.0;
config.lambda_tv_reg = 0.1;

fprintf('Parameters configured:\n');
fprintf('  - pMUT resonance frequency: %.1f kHz\n', params.pmut_resonance_freq/1000);
...
%% ===== STEP 2: GENERATE H MATRIX WITH DIAGNOSTICS =====
fprintf('\n=== STEP 2: GENERATING H MATRIX WITH DIAGNOSTICS ===\n');
[H_matrix, transducer_positions] = generate_sweep_h_matrix_with_diagnostics(params, imaging_grid);
...
function [H_matrix, transducer_positions] = generate_sweep_h_matrix_with_diagnostics(params, imaging_grid)
    % Generate H matrix using verified sweep script approach with diagnostics
