% =========================================================================
% Final Synthesized H-Matrix Parameter Sweep Script v13.0
%
% Description:
% This version implements a powerful form of geometric diversity by
% repositioning the entire virtual transducer grid to a new random location
% for every single acquisition. This is designed to decisively break the
% geometric correlations that caused high coherence in previous versions.
%
% v13.0 Features:
% - For each acquisition, the 9x9 transmit grid is centered at a new random (x,y) offset.
% - The receiver remains fixed at the origin [0,0,0] for a stable reference.
% - Target distance is reduced to 150mm to increase signal strength and
%   highlight the impact of geometric changes.
% =========================================================================
clear; clc; close all;
...
base_config.target_distance_m = 0.150; % MOVED CLOSER to show viability
...
% --- NEW: Define the area over which the TX grid can move ---
base_config.tx_grid_area_m = 0.10; % Grid center can move in a 10x10 cm area
...
disp('--- Initializing Final H-Matrix Sweep with Randomized Grid Positions ---');
session_timestamp = datestr(now, 'mmddyy_HHMMSS');
base_output_dir = fullfile(pwd, 'sweep_output_random_grid_pos');
...
function H = generate_h_matrix_final(config)
    ...
    % We use a 1x1 grid just to define the single receiver element
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % --- NEW: Create a RANDOMLY PLACED virtual grid for THIS acquisition ---
        num_x_grid = 9; num_y_grid = 9;
        total_elements = num_x_grid * num_y_grid;
        
        % Generate a random center for the entire grid
        grid_offset_x = (rand() - 0.5) * config.tx_grid_area_m;
        grid_offset_y = (rand() - 0.5) * config.tx_grid_area_m;
        
        % Define the element centers relative to this new random grid center
        ...
        % The final position of the grid includes the random offset
        grid_center_pos = [grid_offset_x, grid_offset_y, 0];
        TxAperture = xdc_2d_array(num_y_grid, num_x_grid, config.pmut_width_m, config.pmut_width_m, config.kerf_m, config.kerf_m, tx_enabled_matrix, 1, 1, grid_center_pos);
