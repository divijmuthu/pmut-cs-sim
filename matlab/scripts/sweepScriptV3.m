% =========================================================================
% Comprehensive H-Matrix Parameter Sweep Script v6.1 (Definitive Fix)
%
% Description:
...
%
% v6.1 Fixes:
% - Reinstated the DEFINITIVE and physically correct excitation model: a
%   unique, random waveform is generated and assigned to EACH active
%   element in EACH acquisition using the robust `ele_waveform` function.
% - Corrected the element indexing for temporary apertures, which was the
%   root cause of previous failures with this method.
% - This version fully embraces the complexity required for true frequency
%   diversity and will produce meaningful, non-zero coherence results.
% =========================================================================
...
% --- Debug Flag ---
% Set to true to run a small number of tests for fast debugging.
...
% --- If in debug mode, select a small subset of tests ---
if debug_mode
    fprintf('*** DEBUG MODE ACTIVE: Running a reduced number of tests. ***\n');
    % Select first, last, and two middle configurations for variety
    indices_to_run = unique(round(linspace(1, size(param_grid, 1), 4)));
    param_grid = param_grid(indices_to_run, :);
end
...
function H = generate_h_matrix(config)
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % 1. Apodization: Create a vector with size equal to the number of active elements
        ...
        % 2. Excitation: Generate and assign a unique waveform for each active element
        frequencies_hz = config.f_center_hz + (rand(1, num_active_tx) - 0.5) * 2 * config.max_freq_rand_hz;
        for i = 1:num_active_tx
            % The temporary aperture numbers its elements from 1 to num_active_tx
            element_index_in_temp_aperture = i; 
            ...
            % Assign this unique waveform to the element within the temporary aperture
            ele_waveform(TxAperture, element_index_in_temp_aperture, waveform_data);
        end
        
        % 3. Delays: Create a vector with size equal to the number of active elements
