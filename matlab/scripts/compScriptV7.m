% =========================================================================
% Comprehensive H-Matrix Parameter Sweep Script v5.1
%
% Description:
...
%
% v5.1 Fixes:
% - Implemented the definitive, correct procedure for assigning unique
%   waveforms to different elements by calling ele_waveform for each
%   active element, as shown in the Field II User's Guide example.
% - This resolves the persistent "Element_no vector must have one column"
%   error.
% =========================================================================
...
base_config.c = 343;
base_config.fs = 1e6;
base_config.pmut_width_m = 0.005; % Smaller pMUTs for a larger array
base_config.kerf_m = 0.001;       % Kerf between pMUTs
...
p.num_active_tx_sweep = {3, 5}; % Number of *active* transmitters per shot
p.waveform_sweep = {'tukey_sine', 'gaussian_sine'};
p.delay_rand_sweep_us = {0, 12};
p.freq_rand_sweep_hz = {0, 10000};
p.apodization_sweep = {'uniform', 'random', 'hanning'}; % This now applies to the *active* elements
...
% NOTE: Requires 'allcomb.m' from MATLAB File Exchange in the same folder.
...
function H = generate_h_matrix(config)
    ...
    % --- Define a fixed 2D array of potential transmitter locations ---
    num_x_elements = 9;
    num_y_elements = 9;
    total_elements = num_x_elements * num_y_elements;
    
    TxAperture = xdc_2d_array(num_x_elements, num_y_elements, ...
        config.pmut_width_m, config.pmut_width_m, config.kerf_m, config.kerf_m, ...
        ones(num_y_elements, num_x_elements), 1, 1, [0 0 0]);
    
    % --- Define the single-element Receiver ---
    radius = config.pmut_width_m / 2;
    RxAperture = xdc_piston(radius, radius); 
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % --- Randomly select which elements are active for this acquisition ---
        active_indices = randperm(total_elements, num_active_tx);
        
        % --- Define Apodization, Excitation, and Delays for this acquisition ---
        
        % 1. Apodization: Set amplitude for active elements
        apod_master = zeros(1, total_elements);
        switch config.apodization_mode
            case 'uniform', apod_weights = ones(1, num_active_tx);
            case 'random',  apod_weights = rand(1, num_active_tx);
            case 'hanning', apod_weights = hanning(num_active_tx)';
        end
        apod_master(active_indices) = apod_weights;
        xdc_apodization(TxAperture, 0, apod_master);

        % 2. Excitation Waveforms (one for each active element)
        frequencies_hz = config.base_freq_hz + (rand(1, num_active_tx) - 0.5) * 2 * config.max_freq_rand_hz;
        
        % *** DEFINITIVE CORRECTED LOGIC v5.1 ***
        % Assign each unique waveform to its corresponding physical element
        % by calling ele_waveform for each active element, as per the user guide.
        for i = 1:num_active_tx
            physical_index = active_indices(i);
            current_freq = frequencies_hz(i);
            ...
            waveform_data = signal_base .* window * 1e8; % Base amplitude
            
            % Assign this specific waveform to the specific physical element
            ele_waveform(TxAperture, physical_index, waveform_data);
        end
        
        % 3. Delays for the active elements
        delays_master = zeros(1, total_elements);
        delays_us = rand(1, num_active_tx) * config.max_delay_rand_us;
        delays_master(active_indices) = delays_us * 1e-6;
        xdc_focus_times(TxAperture, 0, delays_master);
...
function results = run_diagnostics(name, H, config, output_folder)
    ...
    % To avoid jsonencode error with mixed types, we convert config to a string
