% =========================================================================
% Comprehensive H-Matrix Parameter Sweep Script v6.0 (Debug & Fix)
%
% Description:
...
%
% v6.0 Fixes & Features:
% - Added a `debug_mode` flag to run a small, fast subset of tests.
% - Added new diagnostic plots to visualize H-matrix columns and the full
%   coherence matrix to help debug the "max_coherence = 0" issue.
% - Reverted to the definitive, robust excitation model from the user's
%   reference scripts to ensure physically correct field generation.
% =========================================================================
...
function H = generate_h_matrix(config)
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % 2. Excitation Waveform (a single, random waveform for the whole aperture)
        ...
        % *** DEFINITIVE FIX: Apply impulse AND excitation to the temporary aperture ***
        xdc_impulse(TxAperture, waveform_data);
        xdc_excitation(TxAperture, ones(1, num_active_tx)); % This activates all elements in the temp aperture
