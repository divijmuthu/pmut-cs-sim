% =========================================================================
% Comprehensive H-Matrix Parameter Sweep Script v5.4
%
% Description:
...
%
% v5.4 Fixes:
% - Reinstated true frequency diversity: Each active element in each
%   acquisition now gets a unique, randomly generated waveform.
% - Re-introduced frequency variation as a core parameter in the sweep.
% - Correctly uses ele_waveform to assign unique signals to elements,
%   which is the most robust and physically accurate model.
% - This should resolve the "max_mutual_coherence = 0" issue.
% =========================================================================
...
p.delay_rand_sweep_us = {0, 12, 50};
p.freq_rand_sweep_hz = {0, 10000}; % Max +/- deviation from f_center_hz
...
function H = generate_h_matrix(config)
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % 2. Excitation Waveforms (unique for each active element)
        frequencies_hz = config.f_center_hz + (rand(1, num_active_tx) - 0.5) * 2 * config.max_freq_rand_hz;
