% =========================================================================
% Final Synthesized H-Matrix Parameter Sweep Script v10.0
%
% Description:
% This version introduces the single most powerful method for reducing
% coherence: per-element phase randomization. This technique is designed
% to break the systematic correlations between measurements.
%
% v10.0 Final Features:
% - Implements per-element phase randomization on top of frequency diversity.
% - Uses a pMUT-specific waveform: 3-cycle, Tukey-windowed sine wave.
% - Sweep is focused on spatial and temporal parameters.
% =========================================================================
clear; clc; close all;
...
disp('--- Initializing Final H-Matrix Sweep with Phase Randomization ---');

% --- Setup Output Directory ---
session_timestamp = datestr(now, 'mmddyy_HHMMSS');
base_output_dir = fullfile(pwd, 'sweep_output_final_phase');
...
if debug_mode
    ...
    param_grid = {8, 50, 'random'}; % Use a strong case for debug
end
...
function H = generate_h_matrix_final(config)
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % --- Generate a COMPOSITE WAVEFORM with per-element frequency AND phase diversity ---
        ...
        for k = 1:num_active_tx
            ...
            % *** NEW: Add a random phase shift from 0 to 2*pi ***
