g% =========================================================================
% FINAL REAL DATA RECONSTRUCTION SCRIPT (v3.7 - Stabilized Solver)
%
% Description:
% This definitive version addresses the final challenge of numerical
% instability. It introduces a small amount of Tikhonov regularization
% (damping) directly into the PCG solver. This standard technique stabilizes
% the reconstruction, preventing the "checkerboard" artifacts caused by the
% mismatch between the perfect simulation and noisy real-world data.
%
% This script represents the final, complete, and most robust pipeline.
%
% v3.7 Improvements:
% - Added a `params.pcg_damping_factor` to stabilize the solver.
% - Re-tuned ADMM parameters to work optimally with the stabilized solver.
% =========================================================================

clear; clc; close all;

%% ===== 1. CONFIGURATION AND FILE SELECTION =====
fprintf('=== pMUT FINAL RECONSTRUCTION (v3.7) ===\n');
...
% These parameters are tuned to work with the stabilized solver.
params.lambda_tv_reg = 50.0; % A strong but stable "clean-up" factor
params.rho_admm = 100.0;     % A high penalty to enforce consistency
params.admm_max_iter = 150;

% --- NEW: Solver Stabilization ---
% This is the key to preventing "checkerboard" artifacts.
% It adds a tiny amount of damping to the solver to handle model mismatch.
params.pcg_damping_factor = 1e-4;
...
function reconstructed_image = run_admm_reconstruction(h_components, b_vector, scene_matrix_size, h_norm_factor, config)
    ...
    % *** NEW: Add stabilization term to the PCG solver ***
    Hfun_pcg_admm = @(x_vec) reshape(AtAfun(reshape(x_vec, imageResolution)) + ...
        ...
        config.pcg_damping_factor * x_vec; % Tikhonov Regularization
