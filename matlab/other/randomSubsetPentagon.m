% =========================================================================
% UNIFIED PMUT ULTRASOUND SIMULATION SCRIPT (v1.8 - 5-pMUT Pentagon)
%
% Description:
% This version simulates the "random subset" method using a 5-element
% pentagonal pMUT array. For each acquisition, it randomly selects 3 of the
% 5 available pMUTs to transmit. This provides a much higher degree of
% geometric diversity than the 3-pMUT subset approach, which should
% eliminate the "swirling" reconstruction artifact.
%
% v1.8 Improvements:
% - Simulates a 5-element pentagonal array.
% - Randomly selects a unique subset of 3 out of 5 pMUTs for each shot.
% =========================================================================
clear; clc; close all;

%% ===== 1. MAIN SIMULATION CONFIGURATION =====
fprintf('=== pMUT SIMULATION (v1.8 - 5-pMUT Pentagon) ===\n');

% --- Output Setup ---
timestamp = datestr(now, 'mmddyy_HHMMSS');
output_folder = fullfile('pentagon_simulation_output', timestamp);
...
%% ===== 2. GENERATE H-MATRIX =====
fprintf('\n--- Generating H-Matrix with 5-pMUT Random Subset Selection ---\n');
tic;
...
function [H, imaging_grid] = generate_h_matrix(config)
    ...
    % --- Define the full 5-element physical array ---
    num_x_grid = 11; num_y_grid = 11; % Larger grid to contain the pentagon
    ...
    % Define 5 pMUT positions in a pentagon shape
    radius = 30e-3; % 30mm radius for the pentagon
    tx_desired_positions = zeros(5, 3);
    for i = 1:5
        angle = (i-1) * 2 * pi / 5;
        tx_desired_positions(i, :) = [radius * cos(angle), radius * sin(angle), 0];
    end
    
    tx_physical_indices = zeros(1, 5);
    for i = 1:5, [~, tx_physical_indices(i)] = min(sum((element_centers - tx_desired_positions(i,:)).^2, 2)); end
    ...
    for r_acq = 1:config.num_acquisitions
        % --- *** NEW: Random Subset Selection of 3 from 5 *** ---
        num_to_activate = 3;
        shuffled_indices = randperm(length(tx_physical_indices));
        selected_indices = shuffled_indices(1:num_to_activate);
        active_indices_for_this_shot = tx_physical_indices(selected_indices);
        
        num_active_tx = length(active_indices_for_this_shot);
        ...
        full_apod_vector = zeros(1, num_x_grid*num_y_grid);
        apod_weights = ones(1, num_active_tx);
        if strcmp(config.apodization_mode, 'random'), apod_weights = rand(1, num_active_tx); end
        full_apod_vector(active_indices_for_this_shot) = apod_weights;
        
        full_delay_vector = zeros(1, num_x_grid*num_y_grid);
        full_delay_vector(active_indices_for_this_shot) = delays_us * 1e-6;
