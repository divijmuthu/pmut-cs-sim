...
%
% Description:
% This definitive version combines all successful strategies. It uses the
% memory-safe "forward operator" method to avoid building the ill-conditioned
% H-matrix, and it uses aggressive, correctly-scaled ADMM hyperparameters
% to handle the noisy, powerful real-world data. This is the final,
% most robust pipeline for reconstruction.
%
% v3.5 Improvements:
% - Re-instated the memory-safe forward operator method.
% - Uses aggressive ADMM parameters (`rho_admm`, `lambda_tv_reg`) tuned
%   for real-world data with high signal power.
% =========================================================================
...
% These settings are aggressively tuned for real-world data.
params.rho_admm = 500.0;     % High value to amplify the clean-up term
params.lambda_tv_reg = 2.0;  % A standard clean-up factor
params.admm_max_iter = 150;

% --- Performance & Debugging Settings ---
params.num_acquisitions_to_use = 100s; 
...
params.gate_duration_s = 3.0e-3; % For 4ms capture window
