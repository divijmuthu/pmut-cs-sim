% =========================================================================
% All-in-One Compressed Sensing Reconstruction v8 (Auto-Scaling)
%
% Description:
% This version adds automatic scaling to match the amplitude of the
% simulated H-matrix with the physically measured data. This is critical
...
% --- Configuration: Point to your full-resolution data files ---
DELAY_PROFILE_CSV_FILE = 'pmut_delay_data.csv';
MEASURED_DATA_H5_FILE = 'data_2025-07-02_14-45-47.h5';
...
fprintf('--- All-in-One Reconstruction Script v8 (Auto-Scaling) ---\n');
...
R_acquisitions = size(delay_profiles_us, 1);
...
%% *** NEW: Automatic Scaling Section ***
fprintf('\n--- Automatically Scaling Simulation to Match Measurement ---\n');
% Calculate the power (RMS) of the measured signal
rms_measured = rms(u_measured_signal);
% Estimate the power of the simulated H matrix by checking one column
v_delta = zeros(N_pixels, 1); v_delta(floor(N_pixels/2)) = 1;
h_col_sample = apply_H_operator(v_delta, all_hhp_data, all_start_times, R_acquisitions, total_samples_per_acq, fs);
rms_simulated = rms(h_col_sample);
% Calculate the scaling factor
scaling_factor = rms_measured / rms_simulated;
if isnan(scaling_factor) || isinf(scaling_factor) || scaling_factor == 0
    warning('Invalid scaling factor calculated. Defaulting to 1.');
    scaling_factor = 1;
end
fprintf('Measured RMS: %.2f | Simulated RMS: %.2e | Scaling Factor: %.2e\n', rms_measured, rms_simulated, scaling_factor);

% Create new, scaled operator functions
Afun_scaled = @(v) scaling_factor * apply_H_operator(v, all_hhp_data, all_start_times, R_acquisitions, total_samples_per_acq, fs);
Atfun_scaled = @(u) scaling_factor * apply_HT_operator(u, all_hhp_data, all_start_times, R_acquisitions, total_samples_per_acq, fs, N_pixels);
...
% 1. Plot a raw measured echo signal
subplot(1, 2, 1);
first_acq_data = u_measured_signal(1:total_samples_per_acq);
time_axis_us = (0:total_samples_per_acq-1) / fs * 1e6;
plot(time_axis_us, first_acq_data);
title('Diagnostic: Raw Measured Echo (First Acquisition)');
xlabel('Time (us)'); ylabel('ADC Value'); grid on; axis tight;

% 2. Plot a SCALED simulated impulse response
subplot(1, 2, 2);
h_col_center_scaled = Afun_scaled(v_delta);
plot(time_axis_us, h_col_center_scaled(1:total_samples_per_acq));
title(sprintf('Diagnostic: SCALED Simulated Echo (for center pixel)'));
xlabel('Time (us)'); ylabel('Scaled Amplitude'); grid on; axis tight;
sgtitle('Diagnostic Comparison: Measured vs. SCALED Simulated');
fprintf('Pausing for diagnostic check. Close Figure 98 to continue to reconstruction...\n');
uiwait(gcf);
...
AAtfun_pcg = @(y_vec) Afun_scaled(Atfun_scaled(y_vec));
[y_sol, ~, ~, ~] = pcg(AAtfun_pcg, b_vector(:), tolCG_main, maxItersCG_main);
x_pcg_img_vec = Atfun_scaled(y_sol);
...
Atb_vector = Atfun_scaled(b_vector);
for k_admm = 1:numItersADMM
    ...
    AtAfun_op = @(x) Atfun_scaled(Afun_scaled(x(:)));
