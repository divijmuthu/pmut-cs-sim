% =========================================================================
% FINAL REAL DATA RECONSTRUCTION SCRIPT (v4.0 - v1.5 Alignment)
%
% Description:
% This script is a definitive attempt to reconstruct the real-world data by
% aligning as closely as possible with the successful `v1.5` simulation.
% It abandons the forward operator method in favor of explicitly building
% the full H-matrix to allow for direct analysis, using the memory-safe
% chunking method. It also adopts the proven ADMM hyperparameters from v1.5.
%
% v4.0 Improvements:
% - ADMM parameters (`rho_admm`, `lambda_tv_reg`) are now identical to v1.5.
% - Signal generation logic is more closely aligned with v1.5.
% - Time gate duration is corrected to 3ms for the 4ms capture window.
% - Prioritizes building the full H-matrix for quality and analysis.
% =========================================================================

clear; clc; close all;

%% ===== 1. CONFIGURATION AND FILE SELECTION =====
fprintf('=== pMUT FINAL RECONSTRUCTION (v4.0) ===\n');
...
params.gate_duration_s = 3.0e-3; % Corrected for 4ms capture window
...
H_reshaped = reshape(full(H_un_gated), [num_samples, size(H_un_gated, 2) * params.num_acquisitions]);
H_pruned_reshaped = H_reshaped(gate_mask_single, :);
H_final = sparse(reshape(H_pruned_reshaped, [size(H_pruned_reshaped, 1) * params.num_acquisitions, size(H_un_gated, 2)]));

b_reshaped = reshape(b_vector, [num_samples, params.num_acquisitions]);
b_pruned = b_reshaped(gate_mask_single, :);
b_final = b_pruned(:);
...
fprintf('  Calculating coherence of final pruned H-matrix...');
tic;
[max_coherence_after, coherence_matrix_after] = analyze_coherence(H_final);
fprintf(' Done. (%.2f seconds)\n', toc);
fprintf('  Max Coherence of final matrix: %.4f\n', max_coherence_after);

figure('Visible', 'on', 'Position', [100, 100, 700, 500]);
imagesc(coherence_matrix_after); axis square; colorbar;
title(sprintf('Coherence Matrix of Final Gated H-Matrix\nMax: %.4f', max_coherence_after));
...
function [H, imaging_grid] = generate_h_matrix_from_profiles(config)
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % This signal generation logic is adapted from v1.5
        tx_signals = cell(1, num_active_tx); max_len = 0;
        for k = 1:num_active_tx
            duration = 3 / tx_frequencies(k); % Use a fixed number of cycles like v1.5
            t = 0:1/fs:duration;
        ...
        % Apodization logic from v1.5 (using cycles as weights)
        apodization_weights = config.exp_cycles(r_acq, :) / max(config.exp_cycles(:));
        xdc_apodization(TxAperture, 0, apodization_weights);
    ...
    for c_idx = 1:num_chunks
        ...
        I_list = zeros(total_nnz_chunk, 1);
        J_list = zeros(total_nnz_chunk, 1);
        S_list = zeros(total_nnz_chunk, 1);
        currentIndex = 1;
        for r_acq_local = 1:num_acqs_in_chunk
