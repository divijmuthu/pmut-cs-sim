% =========================================================================
% Explicit H-Matrix Reconstruction v3.0 (DAS + Estimated Impulse)
%
% Description:
% This version takes a fresh approach to improve reconstruction quality.
%
% Key Improvements:
% 1. Estimated Impulse Response: Instead of a synthetic chirp, it now
%    estimates the system's true impulse response from the measured data
%    for a more physically accurate simulation.
% 2. Delay-and-Sum (DAS): Adds a fast, robust DAS beamformer as a
%    baseline reconstruction to verify data quality.
% 3. ADMM Bug Fix: Corrects the "incompatible sizes" error.
% =========================================================================
clearvars;
clc;
close all;
...
% This H5 file should be from a ZERO-DELAY test run to get a clean impulse
IMPULSE_RESPONSE_H5_FILE = 'data_2025-07-07_17-10-00.h5'; % <-- CHANGE THIS
...
set_field('fs', fs_sim);
set_field('c', c);
fprintf('--- Explicit H-Matrix Reconstruction Script (v3.0 - DAS + Estimated Impulse) ---\n\n');
...
% =========================================================================
% *** NEW: Estimate Impulse Response from Data ***
% =========================================================================
fprintf('\n--- Estimating Impulse Response from Measured Data ---\n');
impulse_data_raw = h5read(IMPULSE_RESPONSE_H5_FILE, '/echo_data_raw_adc')';
impulse_data_mv = (double(impulse_data_raw) / max_adc_value) * VOLTAGE_RANGE_MV;
impulse_data_filtered = filter(b_filter, a_filter, impulse_data_mv(1,:)); % Use first acq
[~, peak_idx] = max(abs(impulse_data_filtered));
start_pulse_idx = max(1, peak_idx - round(0.0001 * fs_pico)); % 10us before peak
end_pulse_idx = min(length(impulse_data_filtered), peak_idx + round(0.0001 * fs_pico)); % 10us after
estimated_impulse = impulse_data_filtered(start_pulse_idx:end_pulse_idx);
% Resample to the simulation frequency
t_pico = (0:length(estimated_impulse)-1)/fs_pico;
t_sim = 0:1/fs_sim:t_pico(end);
impulse_response_waveform = interp1(t_pico, estimated_impulse, t_sim, 'linear', 0);
xdc_impulse(Tx_Aperture, impulse_response_waveform);
xdc_excitation(Tx_Aperture, 1);
fprintf('Impulse response estimated and set.\n');
% =========================================================================
...
% =========================================================================
% *** NEW: Delay-and-Sum (DAS) Beamformer ***
% =========================================================================
fprintf('  Calculating Delay-and-Sum (DAS) Image...\n'); tic;
das_image = zeros(N_pixels, 1);
for i = 1:num_active_tx
    % Calculate time-of-flight from this transmitter to every pixel
    tx_pos = tx_positions(i,:);
    tof_tx = vecnorm(hydrophone_positions_img - tx_pos, 2, 2) / c;
    % Calculate time-of-flight from every pixel to the receiver
    tof_rx = vecnorm(hydrophone_positions_img - rx_position, 2, 2) / c;
    total_tof = tof_tx + tof_rx;
    % Convert TOF to sample indices
    sample_indices = round(total_tof * fs_pico);
    
    % Sum the corresponding samples from the measured data
    for p = 1:N_pixels
        if sample_indices(p) > 0 && sample_indices(p) <= total_samples_per_acq
            % This is a simplified DAS for a single acquisition, we average over all
            das_image(p) = das_image(p) + mean(echo_data_matrix(:, sample_indices(p)));
        end
    end
end
das_image = abs(hilbert(das_image)); % Envelope detection
runtime_das = toc; fprintf('  DAS finished in %.2f s.\n', runtime_das);
x_das2D = reshape(das_image, imageResolution);
% =========================================================================
...
for k_admm = 1:numItersADMM
    ...
    % *** CORRECTED: Reshape vector x back to a matrix for opDtDx_op ***
...
subplot(1, 3, 2);
imagesc(x_coords_img, z_coords_img, normalize(x_das2D));
axis image; colormap gray; colorbar; set(gca, 'YDir', 'normal');
title(sprintf('Delay-and-Sum (DAS)\nRuntime: %.2fs', runtime_das));
xlabel('Lateral (m)');
...
sgtitle('Comparison of Reconstructions');
