% =========================================================================
% UNIFIED PMUT ULTRASOUND SIMULATION SCRIPT (v1.20 - Central Anchor Test A)
%
% Description:
% This version implements the "Central Anchor" sensing strategy to mitigate
% swirling artifacts. It uses a fixed core of 4 central transmitters,
% supplemented by a random selection of peripheral transmitters.
%
% This script is configured for Test A:
% - Transmitters: 4 Anchors + 2 Random (Total 6)
% - Receivers: 1 Random
% - Regularization (lambda): Reduced to 5.0 to assess raw improvement.
% =========================================================================
clear; clc; close all;
%% ===== 1. MAIN SIMULATION CONFIGURATION =====
fprintf('=== pMUT SIMULATION (v1.20 - Central Anchor Test A) ===\n');
% --- Output Setup ---
timestamp = datestr(now, 'mmddyy_HHMMSS');
output_folder = fullfile('central_anchor_sim_output', timestamp);
if ~exist(output_folder, 'dir'), mkdir(output_folder); end
fprintf('Saving all results to: %s\n\n', output_folder);
% --- ADMM HYPERPARAMETER TUNING ---
% lambda_tv_reg: The "Clean-Up" or "Denoising" parameter.
params.lambda_tv_reg = 5.0; % Set to 5.0 to assess raw improvement from anchor strategy
    % currnet config: 5 for lambda + 6 sending + 1 receiving
        % quite medicore for 1
        % hwoever 2 is the best so far! 22dB vs the 21.x from earlier nvm
        % we did like >22 before somehwere 22.37 but with 8 tx tho
    % 8 sending, 5 for lamda
        % 18.16 dB for 1 so basically same as earlier
        % slight drop to just under 22 dB for 2nd
% rho_admm: The "Step Size" or "Penalty" parameter.
params.rho_admm = 6.73;
% --- Core Physical and Simulation Parameters ---
params.c = 343;
params.fs = 1e6;
params.pmut_width_m = 0.002; % Correct physical parameter
params.kerf_m = 0.005;       % Correct physical parameter
params.excitation_amplitude = 1e12;
params.target_SNR_db = 35;
% --- H-Matrix Generation Parameters (TEST A CONFIG) ---
params.num_acquisitions = 150;
params.num_active_tx = 8;  % TEST A: 4 Anchors + 2 Random
params.num_active_rx = 1;  % TEST A: 1 Random
...
function [H, imaging_grid] = generate_h_matrix(config)
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % ================================================================
        % --- NEW: Central Anchor Selection Logic (v1.20) ---
        % This logic ensures a stable aperture center to prevent swirling.
        
        % 1. Define fixed anchors and the pool of random elements.
        anchor_tx_elements = [6, 7, 10, 11]; % 4 central pMUTs
        peripheral_elements = setdiff(1:total_elements, anchor_tx_elements);
        num_random_tx_to_add = config.num_active_tx - length(anchor_tx_elements);

        if num_random_tx_to_add < 0
            error('num_active_tx cannot be smaller than the anchor set size.');
        end

        % 2. Build the active transmitter set for this acquisition.
        % Start with the fixed anchors...
        active_tx_indices = anchor_tx_elements;
        % ...and add a random selection from the periphery.
        if num_random_tx_to_add > 0
            random_selection_indices = randperm(length(peripheral_elements), num_random_tx_to_add);
            additional_tx = peripheral_elements(random_selection_indices);
            active_tx_indices = [active_tx_indices, additional_tx];
        end

        % 3. Select receivers from the pool of non-transmitting elements.
        available_for_rx = setdiff(1:total_elements, active_tx_indices);
        if config.num_active_rx > length(available_for_rx)
            error('Not enough available elements to select the desired number of receivers.');
        end
        rx_selection_indices = randperm(length(available_for_rx), config.num_active_rx);
        active_rx_indices = available_for_rx(rx_selection_indices);
        % ================================================================

        num_active_tx = length(active_tx_indices); % This is now always config.num_active_tx
...
function analyze_and_plot_results(reconstructed_image, scene_matrix, H, params, output_folder)
    ...
    xlabel('X Index'); ylabel('Y Index');
    sgtitle(sprintf('Central Anchor (Tx=%d, Rx=%d, lambda=%.1f)', params.num_active_tx, params.num_active_rx, params.lambda_tv_reg));
