% =========================================================================
% UNIFIED PMUT ULTRASOUND SIMULATION SCRIPT (v1.15 - Murata Waveform)
%
% Description:
% This is the definitive "gold standard" simulation. It models a compact
% 4x4 grid of 2mm pMUTs and incorporates a realistic impulse response
% based on the measured waveform from the Murata pMUT presentation. This
% provides the most physically accurate simulation possible to predict the
% performance of a real-world sparse array.
%
% v1.15 Improvements:
% - Replaced the generic sine wave excitation with a realistic, measured
%   impulse response modeled from the Murata pMUT data.
% - Uses the physically correct method of applying a single impulse response
%   and then using apodization and delays for each element.
% =========================================================================
clear; clc; close all;

%% ===== 1. MAIN SIMULATION CONFIGURATION =====
fprintf('=== pMUT SIMULATION (v1.15 - Murata Waveform) ===\n');
...
params = struct();
params.c = 343;
params.fs = 7.81e6; % mediocre with 1e6
...
params.apodization_mode = 'random'; % Use 'random' or 'uniform'
params.f_center_hz = 57e3; % Center frequency from Murata data
...
function [H, imaging_grid] = generate_h_matrix(config)
    ...
    % *** NEW: Define impulse response based on Murata waveform ***
    f0 = config.f_center_hz;
    t_pulse = 0:1/fs:(5/f0); % A 5-cycle pulse
    impulse_response = sin(2*pi*f0*t_pulse) .* tukeywin(length(t_pulse), 0.4)';
    xdc_impulse(FullAperture, impulse_response * config.excitation_amplitude);
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        xdc_impulse(RxAperture, ones(1,10)); % Set receiver to be ideal

        % Apodization and Delays are now applied to the full aperture
        ...
        full_apod_vector(active_tx_indices) = apod_weights;
        full_delay_vector(active_tx_indices) = delays_us * 1e-6;
        
        xdc_apodization(FullAperture, 0, full_apod_vector);
        xdc_focus_times(FullAperture, 0, full_delay_vector);
        
        [h_r, start_time_r] = calc_hhp(FullAperture, RxAperture, grid_points);
...
% (All other helper functions are unchanged from v1.13)
% ...
