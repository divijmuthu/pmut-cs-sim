% =========================================================================
% UNIFIED PMUT ULTRASOUND SIMULATION SCRIPT (v1.17 - Dense Subset)
%
% Description:
% This is the definitive simulation for a 4x4 sparse array. It solves the
% swirling artifact issue by activating a larger, denser random subset of
% 8 pMUTs per acquisition. This stabilizes the "center of energy" of the
% aperture, providing the most robust geometric diversity.
%
% v1.17 Improvements:
% - Increased `num_active_tx` from 5 to 8 for a denser, more stable aperture.
% - Removed the quadrant constraint as it's no longer necessary with a
%   denser subset.
% =========================================================================
clear; clc; close all;

%% ===== 1. MAIN SIMULATION CONFIGURATION =====
fprintf('=== pMUT SIMULATION (v1.17 - Dense Subset) ===\n');
...
% *** NEW: Use a denser subset to stabilize the aperture ***
params.num_active_tx = 8; % original = 8
params.num_active_rx = 1; % original = 1
...
function [H, imaging_grid] = generate_h_matrix(config)
    ...
    for r_acq = 1:config.num_acquisitions
        % *** NEW: Select a dense subset of 8 random elements ***
