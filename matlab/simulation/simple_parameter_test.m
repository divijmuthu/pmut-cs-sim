%% simple_parameter_test.m - Simple test of parameter sweep logic
% Test the core logic without Field II dependencies

clear; clc; close all;

fprintf('=== SIMPLE PARAMETER TEST ===\n');
...
%% Helper Functions (copied from parameter sweep)
...
function reconstructed_image = run_admm_reconstruction(H, b_vector, scene_matrix, config, output_folder)
    ...
    z_admm_iter = zeros(2*N_pixels, 1);
    u_admm_iter = zeros(2*N_pixels, 1);
    
    % ADMM parameters
    rho_admm = config.rho_admm;
    lambda_tv_reg = config.lambda_tv_reg;
    
    % PCG setup for x-update
    Hfun_pcg_admm = @(x) reshape(AtAfun_admm_img(reshape(x, imageResolution)) + rho_admm * opDtDx_tv(reshape(x, imageResolution)), [], 1);
    ...
    for k_admm = 1:config.admm_max_iter
        ...
        % ADMM iteration
        [x_admm_img_iter, z_admm_iter, u_admm_iter] = admm_iteration(x_admm_img_iter, z_admm_iter, u_admm_iter, ...
            Atfun_admm_img, b_vector, opDx_tv, opDtx_tv, rho_admm, lambda_tv_reg, Hfun_pcg_admm, config);
        
        % Calculate metrics
        [PSNR_admm_iters(k_admm), residuals_admm_iters(k_admm)] = calculate_metrics(x_admm_img_iter, v_true_vec_norm, b_vector, Afun_admm, opDx_tv, lambda_tv_reg);
