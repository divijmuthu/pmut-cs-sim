% =========================================================================
% UNIFIED PMUT ULTRASOUND SIMULATION SCRIPT (v3.3 - Digital Twin Version)
%
% Description:
% This version updates the H-matrix generation to be a true "digital twin"
% of the Python/Arduino experiment. It simulates the digital square waves
% and single pre-delay behavior of the physical hardware.
% =========================================================================
clear; clc; close all;

%% ===== 1. MAIN SIMULATION CONFIGURATION =====
fprintf('=== pMUT SIMULATION (v3.3 - Digital Twin Version) ===\n');
...
params.kerf_m = 0.005;
params.excitation_amplitude = 1.0; % Use 1.0 for digital wave
...
%% ===== 3. GENERATE H-MATRIX USING DIGITAL TWIN LOGIC =====
fprintf('\n--- Generating Full H-Matrix using Digital Twin Logic ---\n');
tic;
...
% Plot the coherence matrix for visual inspection
...
function [H, imaging_grid] = generate_h_matrix(config, profiles)
    % This is the UPDATED function that simulates the true experimental physics.
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % --- Define Apertures (using simulation's antipodal/quadrant logic) ---
        ...
        % === STEP 1: Synthesize ideal ANALOG waveform (like Python) ===
        ...
        for k = 1:config.num_active_tx
            duration = 7 / tx_frequencies(k); % Using 7 cycles like Python script
            t_k = 0:1/fs:duration;
            % The Python script used a Blackman window
        ...
        analog_waveform = zeros(1, max_len);
        for k = 1:config.num_active_tx
            sig = tx_signals{k};
            analog_waveform(1:length(sig)) = analog_waveform(1:length(sig)) + sig;
        end
        ...
        digital_waveform = [];
        level = 1; % Start with a positive half-cycle
        for i = 1:length(half_period_samples)
            digital_waveform = [digital_waveform; ones(half_period_samples(i), 1) * level];
            level = -level; % Invert for the next half-cycle
        end

        % === STEP 3: Set excitation with digital wave and simulate ===
        xdc_impulse(TxAperture, digital_waveform' * config.excitation_amplitude);
        ...
        pre_delay_s = mean(delays_us) * 1e-6;
...
% (All other helper functions from the simulation script remain the same)
...
function fingerprints = simulate_fingerprints(delays, freqs, phases, apods, point, config)
    ...
    if any(ismember(active_tx_indices, config.disabled_pmut)) || any(ismember(active_rx_indices, config.disabled_pmut)), error('Fixed geometry conflicts with disabled PMUT.'); end
    tx_enabled_matrix = zeros(num_y_grid, num_x_grid);
    tx_enabled_matrix(active_tx_indices) = 1;
    TxAperture = xdc_2d_array(num_y_grid, num_x_grid, config.pmut_width_m, config.pmut_width_m, config.kerf_m, config.kerf_m, tx_enabled_matrix, 1, 1, [0 0 0]);
    rx_enabled_matrix = zeros(num_y_grid, num_x_grid);
    ...
    for i = 1:num_profiles
        ...
        for k = 1:config.num_active_tx
            ...
            duration = 5 / f_k;
            t_k = 0:1/fs:duration;
            signal_k = apod_weights(k) * sin(2*pi*f_k*t_k + phase_k) .* tukeywin(length(t_k), 0.4)';
            tx_signals{k} = signal_k;
            if length(t_k) > max_len, max_len = length(t_k); end
        end
...
% (Other functions like create_scene, run_forward_simulation, etc. are unchanged)
