% =========================================================================
% UNIFIED PMUT ULTRASOUND SIMULATION SCRIPT (v1.7.1 - No Toolboxes)
%
% Description:
% This version directly tests the hypothesis of using random subset
% selection on a fixed 3-pMUT array to introduce geometric diversity.
%
% v1.7.1 Improvements:
% - Replaced the toolbox-dependent `randsample` function with a standard
%   `randperm` implementation to ensure compatibility with all MATLAB
%   installations.
% =========================================================================
clear; clc; close all;

%% ===== 1. MAIN SIMULATION CONFIGURATION =====
fprintf('=== pMUT SIMULATION (v1.7.1 - Subset Selection) ===\n');

% --- Output Setup ---
timestamp = datestr(now, 'mmddyy_HHMMSS');
output_folder = fullfile('subset_simulation_output', timestamp);
...
%% ===== 2. GENERATE H-MATRIX =====
fprintf('\n--- Generating H-Matrix with Random Subset Selection ---\n');
tic;
...
function [H, imaging_grid] = generate_h_matrix(config)
    ...
    % --- Define the full 3-element physical array ---
    ...
    tx_physical_indices = zeros(1, 3);
    for i = 1:3, [~, tx_physical_indices(i)] = min(sum((element_centers - tx_desired_positions(i,:)).^2, 2)); end
    ...
    for r_acq = 1:config.num_acquisitions
        % --- *** NEW: Random Subset Selection *** ---
        num_to_activate = randi([1, 3]); % Choose to activate 1, 2, or 3 pMUTs
        
        % *** FIX: Replace randsample with randperm for compatibility ***
...
function scene_matrix = create_scene(imaging_grid, params)
    ...
    Y_mm = imaging_grid.Y_mesh * 1000; % Use Y_mesh for top-down view
