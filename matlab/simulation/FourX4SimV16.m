% =========================================================================
% UNIFIED PMUT ULTRASOUND SIMULATION SCRIPT (v1.28 - Disabled Element)
%
% Description:
% This version is modified to account for a single disabled pMUT, creating
% a more accurate digital twin of the real-world hardware. A new parameter,
% `disabled_pmut`, is introduced and excluded from all selections.
% =========================================================================
clear; clc; close all;
%% ===== 1. MAIN SIMULATION CONFIGURATION =====
fprintf('=== pMUT SIMULATION (v1.28 - Disabled Element) ===\n');
% --- Output Setup ---
timestamp = datestr(now, 'mmddyy_HHMMSS');
output_folder = fullfile('disabled_pmut_sim_output', timestamp);
if ~exist(output_folder, 'dir'), mkdir(output_folder); end
fprintf('Saving all results to: %s\n\n', output_folder);

% --- HARDWARE CONFIGURATION ---
params.disabled_pmut = 3; % <-- SET THE INDEX OF THE DISABLED PMUT HERE

% --- ADMM HYPERPARAMETER TUNING ---
params.lambda_tv_reg = 25.0; % 50 actually makes first a bit worse than 25
% also messes up 2nd even more so tuning is key
% base value is like 1.2? also 5 ok just test a bit later
params.rho_admm = 6.73;
% --- Core Physical and Simulation Parameters ---
params.c = 343;
params.fs = 1e6;
params.pmut_width_m = 0.002;
params.kerf_m = 0.015; % 0.005 is usual ~20 dB ok 0.015 is fine kinda better in l1 altho random so consider 
% can optimize profiles + ok 0.015 gave like 28 dB which is great with admm
% lambda of 25
...
function [H, imaging_grid] = generate_h_matrix(config)
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % ================================================================
        % --- Modified Decoupled Antipodal Selection Logic (v1.28) ---
        
        % 1. Define all possible antipodal pairs.
        all_rx_pairs = [ (1:8)', (17-(1:8))' ];
        
        % --- NEW: Filter out the pair containing the disabled pMUT ---
        if ~isempty(config.disabled_pmut)
            rows_to_remove = any(all_rx_pairs == config.disabled_pmut, 2);
            all_rx_pairs(rows_to_remove, :) = [];
        end
        
        % 2. FIRST, select a random pair from the valid list to be the receivers.
        chosen_pair_idx = randi(size(all_rx_pairs, 1));
        active_rx_indices = all_rx_pairs(chosen_pair_idx, :);
        
        % 3. THEN, select transmitters from the remaining available elements.
