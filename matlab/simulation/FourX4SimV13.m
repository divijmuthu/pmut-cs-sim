% =========================================================================
% UNIFIED PMUT ULTRASOUND SIMULATION SCRIPT (v1.25 - Balanced Quadrant Pairs)
%
% Description:
% This version implements the "Balanced Quadrant Pairs" strategy.
% For each acquisition, it activates TWO random elements from each of the
% four quadrants, for a total of 8 transmitters. This ensures the aperture
% is always wide, balanced, and highly variable.
% =========================================================================
clear; clc; close all;
%% ===== 1. MAIN SIMULATION CONFIGURATION =====
fprintf('=== pMUT SIMULATION (v1.25 - Balanced Quadrant Pairs) ===\n');
% --- Output Setup ---
timestamp = datestr(now, 'mmddyy_HHMMSS');
output_folder = fullfile('balanced_quad_sim_output', timestamp);
...
params.num_active_tx = 8;  % Exactly 8, two from each quadrant
...
function [H, imaging_grid] = generate_h_matrix(config)
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % ================================================================
        % --- NEW: Balanced Quadrant Pairs Selection Logic (v1.25) ---
        
        % 1. Define the elements in each quadrant
        quadrants = { ...
            [1, 2, 5, 6],    ... % Top-Left
            [3, 4, 7, 8],    ... % Top-Right
            [9, 10, 13, 14], ... % Bottom-Left
            [11, 12, 15, 16] ... % Bottom-Right
        };
        
        % 2. Select TWO random elements from each quadrant
        active_tx_indices = zeros(1, config.num_active_tx);
        elements_per_quad = config.num_active_tx / 4;
        if mod(config.num_active_tx, 4) ~= 0 || elements_per_quad < 1
            error('num_active_tx must be a multiple of 4 (at least 4).');
        end
        
        currentIndex = 1;
        for q = 1:4
            quad_elements = quadrants{q};
            % randperm to select without replacement
        ...
        % 3. Select receivers from the remaining elements
...
function analyze_and_plot_results(reconstructed_image, scene_matrix, H, params, output_folder)
    ...
    xlabel('X Index'); ylabel('Y Index');
    sgtitle(sprintf('Balanced Quadrant Pairs (Tx=%d, Rx=%d, lambda=%.1f)', params.num_active_tx, params.num_active_rx, params.lambda_tv_reg));
