% =========================================================================
% UNIFIED PMUT ULTRASOUND SIMULATION SCRIPT (v1.19 - ADMM Tuning)
%
% Description:
...
% standard method for improving image quality when working with noisy or
% artifact-prone data.
%
% v1.19 Improvements:
...
fprintf('=== pMUT SIMULATION (v1.19 - ADMM Tuning) ===\n');
...
%   - Higher values (e.g., 10, 15) make the algorithm more aggressive
...
% original was 1.2
params.lambda_tv_reg = 25.0; % RECOMMENDATION: Increase this value or
% 15 led to pretty good reconstructions, also check more transmit+receive
% increase more & adjust rho
% 8 sending, 2 receiving:
    % 1st better 2nd worse with 15
    % 25 a little better for 1st BUT made 2nd worse again...
    % possibly future optimization make sure to tune the ADMM params per
    % img honestly acceptable for 3 with 25
% 8 sending, 1 receiving, 25
    % 1 was worse, so we should probably aim for more receiving
    % however 2 better? tricky...
% 8 sending 4 receiving 25
    % 1 was better closing in on 20 dB decently recognizables 
    % 2 is worst tho
...
params.num_active_tx = 8; % initially 8
params.num_active_rx = 4; % check 2 receiving case?
...
function [H, imaging_grid] = generate_h_matrix(config)
    ...
    for r_acq = 1:config.num_acquisitions
        all_indices = randperm(total_elements);
        active_tx_indices = all_indices(1:config.num_active_tx);
        active_rx_indices = all_indices(config.num_active_tx + 1 : config.num_active_tx + config.num_active_rx);
