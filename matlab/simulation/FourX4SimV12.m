% =========================================================================
% UNIFIED PMUT ULTRASOUND SIMULATION SCRIPT (v1.21 - Binary Apodization)
%
% Description:
% This version tests the "Binary Random" apodization strategy. Instead of
% assigning random amplitudes between 0 and 1, active transmitters are
% forced to be either fully ON (1) or OFF (0). This creates higher
% contrast beam patterns to improve decorrelation.
%
% This script is configured with the 8Tx/1Rx Central Anchor geometry.
% =========================================================================
clear; clc; close all;
%% ===== 1. MAIN SIMULATION CONFIGURATION =====
fprintf('=== pMUT SIMULATION (v1.21 - Binary Apodization) ===\n');
% --- Output Setup ---
timestamp = datestr(now, 'mmddyy_HHMMSS');
output_folder = fullfile('binary_apod_sim_output', timestamp);
if ~exist(output_folder, 'dir'), mkdir(output_folder); end
fprintf('Saving all results to: %s\n\n', output_folder);
% --- ADMM HYPERPARAMETER TUNING ---
params.lambda_tv_reg = 5.0; % Kept at 5.0 for direct comparison
...
params.num_active_tx = 8;  % Using 8 Tx (4 Anchors + 4 Random)
params.num_active_rx = 1;  % Using 1 Rx
params.max_delay_rand_us = 500; % Kept at 500us due to capture window constraint
params.apodization_mode = 'binary_random'; % <-- NEW STRATEGY
...
function [H, imaging_grid] = generate_h_matrix(config)
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % --- Central Anchor Selection Logic ---
        anchor_tx_elements = [6, 7, 10, 11];
        ...
        if num_random_tx_to_add < 0, error('num_active_tx cannot be smaller than the anchor set size.'); end
        ...
        if config.num_active_rx > length(available_for_rx), error('Not enough available elements for receivers.'); end
        ...
        % ================================================================
        % --- NEW: Apodization Logic (v1.21) ---
        if strcmp(config.apodization_mode, 'binary_random')
            apod_weights = rand(1, num_active_tx) > 0.5;
            % Ensure at least one transmitter is on to avoid a null signal
            if all(apod_weights == 0)
                apod_weights(randi(num_active_tx)) = 1;
            end
        elseif strcmp(config.apodization_mode, 'random')
            apod_weights = rand(1, num_active_tx);
        else % Default to uniform apodization
...
function analyze_and_plot_results(reconstructed_image, scene_matrix, H, params, output_folder)
    ...
    xlabel('X Index'); ylabel('Y Index');
    sgtitle(sprintf('%s (Tx=%d, Rx=%d, lambda=%.1f)', strrep(params.apodization_mode, '_', ' '), params.num_active_tx, params.num_active_rx, params.lambda_tv_reg));
