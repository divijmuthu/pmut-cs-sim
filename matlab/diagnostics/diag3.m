% =========================================================================
% Comprehensive Data Diagnostic Script v4.5 (Peak Envelope Scaling)
%
% Description:
% This version changes the amplitude scaling method. Instead of matching
% RMS power, it now scales the simulation to match the peak of the
% measured signal's envelope for a more intuitive visual comparison.
% =========================================================================
clearvars;
clc;
close all;
...
DELAY_PROFILE_CSV_FILE = 'delays_2025-07-11_14-38-54.csv'; %'delays_2025-07-08_15-16-12.csv'; %'delays_2025-07-07_17-03-00.csv';
MEASURED_DATA_H5_FILE = 'data_2025-07-11_14-38-54.h5'; %'data_2025-07-08_15-16-12.h5'; % 'data_2025-07-07_17-03-00.h5'
...
PLOT_TIME_RANGE_US = [-500, 9500]; % Set plot time range [start, end] in microseconds
...
% --- Calculate the TRUE sampling rate from known parameters ---
total_duration_s = 10e-3; % The known total acquisition time is 10 ms
fs = total_samples / total_duration_s; % fs = samples / time
fprintf('Calculated true sampling rate based on %d samples over %.1f ms: %.2f MHz\n', total_samples, total_duration_s*1e3, fs/1e6);

% --- Core Physical and Simulation Constants ---
c = 1540;                   % Speed of Sound [m/s]
set_field('fs', fs);        % USE THE CORRECT, CALCULATED SAMPLING RATE
set_field('c', c);

fprintf('--- Comprehensive Diagnostic Script v4.5 (Peak Envelope Scaling) ---\n');
fprintf('Analyzing data at the true %.2f MHz sampling rate.\n\n', fs/1e6);
...
% --- Define target position based on measured echo arrival time ---
target_pos = [0, 0, 0.1463]; % A single point target at 14.63cm depth
fprintf('Adjusted simulated target distance to %.2f cm to match measured data.\n', target_pos(3)*100);
...
% *** FIX: Amplitude scaling based on peak of the envelope for better visual comparison ***
env_measured = abs(hilbert(u_measured_single_filtered));
env_simulated = abs(hilbert(u_simulated_filtered));

scaling_factor = max(env_measured) / max(env_simulated);
