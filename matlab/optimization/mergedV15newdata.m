% =========================================================================
% REAL DATA RECONSTRUCTION SCRIPT (v2.8 - Data Transpose Fix)
%
% Description:
% This version fixes a critical bug caused by a dimension order mismatch
% between how Python (h5py) saves data and how MATLAB (h5read) loads it.
%
% v2.8 Improvements:
% - Transposes the raw data matrices immediately after loading from HDF5
%   to ensure the dimensions are (samples, acquisitions), matching the
%   rest of the script's expectations and fixing the mismatch error.
% =========================================================================

clear; clc; close all;

%% ===== 1. CONFIGURATION AND FILE SELECTION =====
fprintf('=== pMUT REAL DATA RECONSTRUCTION (v2.8) ===\n');
...
params.fs = 1 / sample_interval_s;
fprintf('  Detected experimental sampling rate: %.2f MHz\n', params.fs / 1e6);

% *** FIX: Transpose data to correct for Python/MATLAB dimension order mismatch ***
% Data is loaded as (acqs, samples), so we transpose to (samples, acqs).
target_data = target_data_raw';
fprintf('  Transposed target data matrix to correct orientation.\n');
...
if ~isempty(background_filepath)
    ...
    background_data = background_data_raw'; % Transpose background data as well
...
try
    % lowpass works on columns, data is now correctly oriented (samples, acqs)
    filtered_data = lowpass(processed_data, params.filter_cutoff_hz, params.fs);
catch ME
...
% --- Finalize measurement vector and load profiles ---
b_vector = filtered_data(:);
num_samples_per_acq = size(filtered_data, 1);
params.num_acquisitions = size(filtered_data, 2);

fprintf('  Loading CSV profiles (skipping header row)...\n');
params.exp_frequencies_hz = readmatrix(freq_filepath, 'NumHeaderLines', 1);
params.exp_delays_us = readmatrix(delay_filepath, 'NumHeaderLines', 1);
params.exp_phases_deg = readmatrix(phase_filepath, 'NumHeaderLines', 1);

% --- Detailed Data Validation ---
fprintf('  Validating data dimensions...\n');
fprintf('    Found %d acquisitions in HDF5 data.\n', params.num_acquisitions);
fprintf('    Found %d rows in Frequencies CSV.\n', size(params.exp_frequencies_hz, 1));
fprintf('    Found %d rows in Delays CSV.\n', size(params.exp_delays_us, 1));
fprintf('    Found %d rows in Phases CSV.\n', size(params.exp_phases_deg, 1));

if size(params.exp_frequencies_hz, 1) ~= params.num_acquisitions || ...
   size(params.exp_delays_us, 1) ~= params.num_acquisitions || ...
   size(params.exp_phases_deg, 1) ~= params.num_acquisitions
    error(['Mismatch between number of acquisitions in HDF5 file and CSV profile files.\n' ...
           'This can happen if a data collection run was interrupted.']);
end
...
if num_h_samples_per_acq ~= num_samples_per_acq
    fprintf('  Aligning H and b vectors (H samples: %d, b samples: %d)\n', ...
            num_h_samples_per_acq, num_samples_per_acq);
    min_samples = min(num_h_samples_per_acq, num_samples_per_acq);
    H_reshaped = reshape(full(H), [num_h_samples_per_acq, size(H, 2), params.num_acquisitions]);
    b_reshaped = reshape(b_vector, [num_samples_per_acq, 1, params.num_acquisitions]);
...
%% ===== HELPER FUNCTIONS (Identical to v2.6, included for completeness) =====
