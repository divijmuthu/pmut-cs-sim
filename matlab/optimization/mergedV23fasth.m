% =========================================================================
% REAL DATA RECONSTRUCTION SCRIPT (v2.17 - Fast H-Matrix Assembly)
%
% Description:
% This version dramatically improves the performance of the H-matrix
% assembly step, which was the primary bottleneck. Instead of dynamically
% inserting blocks into a sparse matrix, it now uses the highly efficient
% "triplet" method (collecting I, J, S indices) and constructs the final
% sparse matrix in a single, optimized operation.
%
% v2.17 Improvements:
% - Re-architected the H-matrix assembly loop to be significantly faster.
% - Removed the slow, iterative insertion of data blocks.
% =========================================================================

clear; clc; close all;

%% ===== 1. CONFIGURATION AND FILE SELECTION =====
fprintf('=== pMUT REAL DATA RECONSTRUCTION (v2.17) ===\n');
...
function [H, imaging_grid] = generate_h_matrix_from_profiles(config)
    ...
    % --- *** NEW: Fast H-Matrix Assembly using Triplet Method *** ---
    disp('  Assembling H-matrix using fast triplet method...');
    ...
    % Pre-allocate for the triplet lists (I, J, S)
    % Estimate the number of non-zero elements to pre-allocate memory
    estimated_nnz = sum(all_K_values) * N_pixels; 
    I_list = zeros(estimated_nnz, 1);
    J_list = zeros(estimated_nnz, 1);
    S_list = zeros(estimated_nnz, 1);
    
    currentIndex = 1;
    wb_asm = waitbar(0, 'Preparing H-matrix triplets...');
    for r_acq = 1:config.num_acquisitions
        if all_K_values(r_acq) > 0 && ~isempty(all_h_data{r_acq})
            ...
            % Offset row indices for the full H matrix
    ...
    % Trim any excess pre-allocated space
