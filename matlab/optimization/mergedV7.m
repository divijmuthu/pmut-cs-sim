% =========================================================================
% UNIFIED PMUT ULTRASOUND SIMULATION SCRIPT (v1.5)
%
% Description:
% This version transforms the single simulation into a sequential sweep that
% tests the system's performance against multiple, complex target scenes.
%
% v1.5 Improvements:
% - Enabled random apodization to add another layer of signal diversity.
% - Significantly increased the number of acquisitions to 150 for a more
%   data-rich reconstruction.
% =========================================================================
clear; clc; close all;
...
% --- H-Matrix Generation Parameters (Set once for the whole sweep) ---
% IMPROVEMENT: Increased number of acquisitions for a more robust H-matrix
params.num_acquisitions = 150;     % (Original: 50)
params.num_active_tx = 5;
params.max_delay_rand_us = 500;
% IMPROVEMENT: Enabled random apodization for more signal diversity
params.apodization_mode = 'random'; % (Original: 'uniform')
params.tx_pool_width_m = 0.300;
params.f_min_hz = 45e3;
params.f_max_hz = 65e3;

% --- Imaging Grid Parameters (Set once for the whole sweep) ---
...
fprintf('\n--- Generating H-Matrix (once for all scenes) ---\n');
tic;
...
for i = 1:length(target_patterns_to_test)
    ...
    % --- Update params for the current scene ---
