...
params.f_min_hz = 35e3;
params.f_max_hz = 100e3;
...
%% ===== 2. GENERATE AND SAVE EXAMPLE SIGNAL FIGURE =====
fprintf('\n--- Generating Example Signal Figure ---\n');

% Generate example signal parameters
example_freqs = params.f_min_hz + (params.f_max_hz - params.f_min_hz) * rand(1, params.num_active_tx);
example_phases = params.max_phase_rad * rand(1, params.num_active_tx);
example_delays = params.max_delay_rand_us * rand(1, params.num_active_tx);
example_apods = params.min_apod + (params.max_apod - params.min_apod) * rand(1, params.num_active_tx);

% Generate individual signals
fs = params.fs;
max_len = 0;
tx_signals = cell(1, params.num_active_tx);
        for k = 1:params.num_active_tx
            f_k = example_freqs(k);
            phase_k = example_phases(k);
            
            % Use fixed duration for all signals to ensure consistent length
            duration = 10 / 50e3; % Use 50 kHz as reference
            t_k = 0:1/fs:duration;
            num_samples = length(t_k);
            
            % Create amplitude envelope: ramp up for first 2 cycles, ramp down for remaining 8 cycles
            ramp_up_samples = round(0.2 * num_samples);
            ramp_down_samples = num_samples - ramp_up_samples;
            
            % Create envelope
            envelope = zeros(1, num_samples);
            envelope(1:ramp_up_samples) = linspace(0, 1, ramp_up_samples);
            envelope(ramp_up_samples+1:end) = linspace(1, 0, ramp_down_samples);
            
            % Generate signal
            signal_k = sin(2*pi*f_k*t_k + phase_k) .* envelope;
...
% Create time axis
t_axis = (0:length(composite_waveform)-1) / fs * 1000; % Convert to milliseconds
...
for k = 1:params.num_active_tx
    sig = tx_signals{k};
    t_sig = (0:length(sig)-1) / fs * 1000;
    plot(t_sig, sig + k*0.5, 'LineWidth', 1.5);
    hold on;
end
xlabel('Time (ms)');
ylabel('Amplitude (offset)');
title('Individual Transmitter Signals (10 cycles: 2 ramp-up + 8 ramp-down)');
legend(arrayfun(@(x) sprintf('Tx%d: %.1f kHz', x, example_freqs(x)/1000), 1:params.num_active_tx, 'UniformOutput', false), 'Location', 'eastoutside');
grid on;

% Plot composite waveform
subplot(2,2,2);
plot(t_axis, composite_waveform, 'LineWidth', 2, 'Color', 'red');
xlabel('Time (ms)');
ylabel('Amplitude');
title('Composite Waveform (Sum of All Signals)');
grid on;

% Plot frequency spectrum
subplot(2,2,3);
N = length(composite_waveform);
freq_axis = (0:N-1) * fs / N / 1000; % Convert to kHz
spectrum = abs(fft(composite_waveform));
plot(freq_axis(1:floor(N/2)), spectrum(1:floor(N/2)), 'LineWidth', 2);
xlabel('Frequency (kHz)');
ylabel('Magnitude');
title('Frequency Spectrum of Composite Signal');
xlim([0, 120]); % Show up to 120 kHz
grid on;

% Plot signal parameters
subplot(2,2,4);
bar(1:params.num_active_tx, example_freqs/1000, 'FaceColor', 'blue');
hold on;
bar(1:params.num_active_tx, example_delays/1000, 'FaceColor', 'red');
xlabel('Transmitter Index');
ylabel('Frequency (kHz) / Delay (ms)');
title('Signal Parameters per Transmitter');
legend('Frequency (kHz)', 'Delay (ms)', 'Location', 'northeast');
grid on;

% Add text box with parameters
text_str = sprintf(['Signal Parameters:\n', ...
                   'Frequency Range: %.1f - %.1f kHz\n', ...
                   'Pulse Length: 10 cycles (2 ramp-up + 8 ramp-down)\n', ...
                   'Sampling Rate: %.1f MHz\n', ...
                   'Number of Transmitters: %d\n', ...
                   'Apodization Range: %.1f - %.1f'], ...
                   params.f_min_hz/1000, params.f_max_hz/1000, ...
                   params.fs/1e6, params.num_active_tx, ...
                   params.min_apod, params.max_apod);

annotation('textbox', [0.02, 0.02, 0.3, 0.15], 'String', text_str, ...
           'EdgeColor', 'black', 'BackgroundColor', 'white', 'FontSize', 10);

sgtitle('Example pMUT Signal Generation (10 cycles: 2 ramp-up + 8 ramp-down, 35-100 kHz)', 'FontSize', 16, 'FontWeight', 'bold');

% Save figure
saveas(gcf, fullfile(main_output_folder, 'example_signal_generation.png'));
saveas(gcf, fullfile(main_output_folder, 'example_signal_generation.fig'));
close(gcf);
fprintf('Example signal figure saved to: %s/\n', main_output_folder);

%% ===== 3. GENERATE OPTIMAL PROFILES (GREEDY SEARCH) =====
...
%% ===== 4. GENERATE H-MATRIX USING OPTIMIZED PROFILES =====
...
%% ===== 5. SEQUENTIAL SCENE SIMULATION SWEEP =====
