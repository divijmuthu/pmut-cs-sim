% =========================================================================
% REAL DATA RECONSTRUCTION SCRIPT (v2.23 - Forward Operator Method)
%
% Description:
% This version implements the definitive solution to the out-of-memory
% errors by using a "forward operator" approach. The full, massive H-matrix
% is never formed in memory. Instead, functions are created that compute
% the action of H and its transpose on-the-fly, one acquisition at a time.
% This is the most memory-efficient and scalable solution.
%
% v2.23 Improvements:
% - Re-architected the script to use function handles (Afun, Atfun) instead
%   of an explicit H matrix.
% - `generate_h_matrix_components` now only simulates the building blocks.
% - ADMM solver now uses these memory-efficient function handles.
% - Coherence analysis is removed as the full H is never formed.
% =========================================================================

clear; clc; close all;

%% ===== 1. CONFIGURATION AND FILE SELECTION =====
fprintf('=== pMUT REAL DATA RECONSTRUCTION (v2.23) ===\n');
...
params.downsample_factor = 4; % We can use less downsampling now
...
tic;
reconstructed_image = run_admm_reconstruction(h_components, b_final, zeros(size(imaging_grid.X_mesh)), params);
...
function reconstructed_image = run_admm_reconstruction(h_components, b_vector, scene_matrix_size, config)
    imageResolution = size(scene_matrix_size);
    
    % --- Create Forward Operator Function Handles ---
    [Afun, Atfun, AtAfun] = operator_setup(h_components, imageResolution, config);
    
    b_vec_norm = b_vector(:); % No normalization needed as H is not formed
    ...
    for k_admm = 1:config.admm_max_iter
        ...
        bb_upd = Atfun(b_vec_norm) + config.rho_admm * opDtx_tv(v_upd);
        [x_vec_new, ~, ~, ~] = pcg(Hfun_pcg_admm, bb_upd(:), config.pcg_tol, config.pcg_max_iter, [], [], x_admm_img_iter(:));
...
function [Afun, Atfun, AtAfun] = operator_setup(h_components, imageResolution, config)
    ...
    function y = forward_op(x_img)
        ...
        for i = 1:num_acqs
            ...
            y(start_idx:end_idx) = h_components{i} * x_vec;
        end
    end
    ...
    function x_img = transpose_op(y_vec)
        ...
        for i = 1:num_acqs
            x_vec = x_vec + (h_components{i}' * y_reshaped(:, i));
        end
...
function analyze_and_plot_reconstruction(reconstructed_image, imaging_grid, output_folder)
    % Coherence is no longer calculated, so it's removed from the plot
