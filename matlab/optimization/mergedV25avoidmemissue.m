% =========================================================================
% REAL DATA RECONSTRUCTION SCRIPT (v2.19 - Chunked H-Matrix Assembly)
%
% Description:
% This version solves the out-of-memory error by implementing a robust
% "chunking" method for H-matrix assembly. Instead of building the entire
% massive matrix at once, it processes the acquisitions in smaller chunks,
% building a partial H-matrix for each. These smaller matrices are then
% efficiently combined at the end. This approach is both memory-safe and
% fast.
%
% v2.19 Improvements:
% - Implemented chunked H-matrix assembly to prevent out-of-memory errors.
% - Added a new parameter `params.assembly_chunk_size`.
% =========================================================================

clear; clc; close all;

%% ===== 1. CONFIGURATION AND FILE SELECTION =====
fprintf('=== pMUT REAL DATA RECONSTRUCTION (v2.19) ===\n');
...
params.assembly_chunk_size = 25; % Process N acquisitions at a time
...
function [H, imaging_grid] = generate_h_matrix_from_profiles(config)
    ...
    % --- *** NEW: Fast and Memory-Safe H-Matrix Assembly (Chunked) *** ---
    ...
    for c_idx = 1:num_chunks
        ...
        % Pass 1 for this chunk: Count nnz
        total_nnz_chunk = 0;
        ...
        % Pass 2 for this chunk: Build triplets and sparse matrix
    ...
    % Combine the chunks vertically to form the final H matrix
    H = vertcat(H_chunks{:});
