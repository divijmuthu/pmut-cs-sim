% =========================================================================
% REAL DATA RECONSTRUCTION SCRIPT (v2.11 - Corrected Geometry)
%
% Description:
% This version corrects the simulation geometry to match the physical
% experiment. The imaging grid is now defined as an XY-plane at a fixed
% height Z, representing a "top-down" view of the target hanging above
% the pMUT array.
%
% v2.11 Improvements:
% - Corrected imaging grid definition from XZ-plane to XY-plane.
% - Renamed configuration parameters for clarity (e.g., target_height_m).
% - Updated plotting function to display the reconstruction with X and Y axes.
% =========================================================================

clear; clc; close all;

%% ===== 1. CONFIGURATION AND FILE SELECTION =====
fprintf('=== pMUT REAL DATA RECONSTRUCTION (v2.11) ===\n');
...
params.grid_x_width_m = 0.150;     % Width of the imaging area in X
params.grid_y_width_m = 0.150;     % Width of the imaging area in Y
params.target_height_m = 0.150;    % Height (Z-distance) of the imaging plane
...
function [H, imaging_grid] = generate_h_matrix_from_profiles(config)
    ...
    % *** GEOMETRY FIX: Define an XY imaging grid at a constant height Z ***
...
function analyze_and_plot_reconstruction(reconstructed_image, imaging_grid, H, params, output_folder)
    ...
    ylabel('Y Position (mm)'); % Corrected label
