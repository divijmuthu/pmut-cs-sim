% =========================================================================
% REAL DATA RECONSTRUCTION SCRIPT (v2.12 - Time Gating)
%
% Description:
% This version adds a crucial time-gating step to remove the "ghost image"
% artifact caused by transmitter-receiver crosstalk. By zeroing out the
% early-time data, the algorithm is forced to focus on the later-arriving
% echoes from the actual target.
%
% v2.12 Improvements:
% - Added `params.gate_start_time_s` and `params.gate_duration_s` to
%   define a window of interest.
% - Implemented a time-gating mask that is applied to the pre-processed
%   data to remove crosstalk before reconstruction.
% =========================================================================

clear; clc; close all;

%% ===== 1. CONFIGURATION AND FILE SELECTION =====
fprintf('=== pMUT REAL DATA RECONSTRUCTION (v2.12) ===\n');
...
params.downsample_factor = 16; % Using a moderate downsample for speed
...
% --- NEW: Time Gating Parameters ---
% We expect the echo to arrive after traveling to the target and back.
% We add a small buffer and define a window around this time.
params.gate_start_time_s = (params.target_height_m / params.c) * 0.8; % Start gate just before expected echo
params.gate_duration_s = 1.5e-3; % Gate for 1.5 ms
...
gated_data = filtered_data .* gate_mask'; % Apply mask to each column
...
num_acqs_available = size(gated_data, 2);
...
final_data = gated_data(:, 1:params.num_acquisitions);
