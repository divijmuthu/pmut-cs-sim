% =========================================================================
% REAL DATA RECONSTRUCTION SCRIPT (v2.15 - Acquisition Selection)
%
% Description:
% This version allows the user to explicitly select a subset of acquisitions
% to use for reconstruction. This is a powerful diagnostic tool to test
% hypotheses by excluding known "bad" or correlated data blocks.
%
% v2.15 Improvements:
% - Added `params.acquisitions_to_use` to specify which acquisition
%   indices to include in the analysis and reconstruction.
% =========================================================================

clear; clc; close all;

%% ===== 1. CONFIGURATION AND FILE SELECTION =====
fprintf('=== pMUT REAL DATA RECONSTRUCTION (v2.15) ===\n');
...
% *** NEW: Select which acquisitions to use. This allows us to exclude the
% known correlated block from acquisitions ~20-90. ***
params.acquisitions_to_use = [1:19, 91:150]; % Exclude the "bad" block
...
if ~exist(freq_filepath, 'file') || ~exist(delay_filepath, 'file') || ~exist(phase_filepath, 'file')
    error('Could not automatically find all matching CSV files.');
end
...
params.gate_duration_s = 8e-3; % Using your optimized 8ms duration
...
% --- Select the specified subset of acquisitions ---
fprintf('  Selecting %d specified acquisitions...\n', length(params.acquisitions_to_use));
final_data = filtered_data(:, params.acquisitions_to_use);
params.exp_frequencies_hz = all_freqs(params.acquisitions_to_use, :);
params.exp_delays_us = all_delays(params.acquisitions_to_use, :);
params.exp_phases_deg = all_phases(params.acquisitions_to_use, :);
params.num_acquisitions = size(final_data, 2);
...
gated_data = final_data .* gate_mask';
...
num_b_samples_per_acq = size(gated_data, 1);
...
tic;
reconstructed_image = run_admm_reconstruction(H_gated, b_vector, zeros(size(imaging_grid.X_mesh)), params);
...
%% ===== HELPER FUNCTIONS =====
% All helper functions (generate_h_matrix, ADMM, plotting, etc.) are
% identical to v2.14 and are included below for completeness.
...
function [H, imaging_grid] = generate_h_matrix_from_profiles(config)
    ...
    for r_acq = 1:config.num_acquisitions
        if mod(r_acq, 10) == 0 || r_acq == config.num_acquisitions, fprintf('  Assembling acquisition %d/%d (%.0f%%)...\n', r_acq, config.num_acquisitions, (r_acq/config.num_acquisitions)*100); end
        if all_K_values(r_acq) > 0 && ~isempty(all_h_data{r_acq})
            ...
            if max(row_indices) <= size(H, 1), H(row_indices, :) = h_aligned; end
        end
        current_row_offset = current_row_offset + K_global_per_acq;
        waitbar(r_acq / config.num_acquisitions, wb_asm);
    end
    close(wb_asm);
