% =========================================================================
% REAL DATA RECONSTRUCTION SCRIPT (v2.13 - H-Matrix Diagnostics)
%
% Description:
% This version adds detailed diagnostic steps to analyze the simulated
% H-matrix before and after time-gating. This allows us to verify that
% the pre-processing is successfully reducing coherence by removing the
% simulated crosstalk artifact, aligning the digital twin with the
% properties of our successful earlier simulations.
%
% v2.13 Improvements:
% - Calculates and prints the coherence of the H-matrix BEFORE time-gating.
% - Explicitly applies the time-gate mask to the H-matrix.
% - Calculates and prints the coherence of the H-matrix AFTER time-gating.
% - Generates a new plot comparing the coherence matrices visually.
% =========================================================================

clear; clc; close all;

%% ===== 1. CONFIGURATION AND FILE SELECTION =====
fprintf('=== pMUT REAL DATA RECONSTRUCTION (v2.13) ===\n');
...
if num_h_samples_per_acq ~= num_b_samples_per_acq
    ...
    b_reshaped = reshape(final_data, [num_b_samples_per_acq, 1, params.num_acquisitions]);
...
H_gated = H_un_gated .* gate_mask_full; % Element-wise multiplication applies mask to each column

[max_coherence_after, coherence_matrix_after] = analyze_coherence(H_gated);
fprintf('  Max Coherence AFTER time-gating: %.4f (Low coherence is the goal)\n', max_coherence_after);

% --- Visualize Coherence Matrices ---
figure('Visible', 'on', 'Position', [100, 100, 1200, 500]);
subplot(1, 2, 1);
imagesc(coherence_matrix_before); axis square; colorbar;
...
function [H, imaging_grid] = generate_h_matrix_from_profiles(config)
    % This function is unchanged from v2.12
    fs = config.fs;
    c = config.c;
...
function [max_coherence, coherence_matrix] = analyze_coherence(H)
    ...
    if ~isempty(Hn)
        ...
        coherence_matrix(1:size(coherence_matrix,1)+1:end) = 0; % Zero out the diagonal
...
function analyze_and_plot_reconstruction(reconstructed_image, imaging_grid, max_coherence, output_folder)
    % This function now takes the final max_coherence as an input
