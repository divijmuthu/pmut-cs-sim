% =========================================================================
% FINAL REAL DATA RECONSTRUCTION SCRIPT (v3.0 - Forward Operator)
%
% Description:
% This is the definitive, memory-safe reconstruction script. It combines
% all successful strategies into a single, robust pipeline. The core
% improvement is the use of a "forward operator" method, which avoids
% building the massive H-matrix in memory, thus preventing crashes.
%
% This script is designed to run on the final, optimized, apodized dataset.
%
% Key Features:
% - Memory-Safe Forward Operator: Calculates H*x and H'*y on-the-fly.
% - Full Pre-Processing: Background subtraction, filtering, and time-gating.
% - Digital Twin Accuracy: Simulates the exact experimental profiles,
%   including random apodization via cycle counts.
% =========================================================================

clear; clc; close all;

%% ===== 1. CONFIGURATION AND FILE SELECTION =====
fprintf('=== pMUT FINAL RECONSTRUCTION (v3.0) ===\n');
...
params.num_acquisitions_to_use = 30; % Use Inf for all, or set a number (e.g., 75) for a faster test
params.downsample_factor = 4;       % 4 is a good balance; 8 is faster, 2 is higher quality
...
params.grid_step_m = 0.003; % Slightly coarser grid for faster ADMM
