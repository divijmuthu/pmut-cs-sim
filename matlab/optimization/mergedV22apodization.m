% =========================================================================
% REAL DATA RECONSTRUCTION SCRIPT (v2.16 - Apodization & Analysis Mode)
%
% Description:
% This version is the full digital twin for the apodization experiment. It
% loads the new `cycles` data and incorporates it into the H-matrix
% simulation by modulating the amplitude of each transducer signal. It is
% also configured to run in an "analysis-only" mode by default.
%
% v2.16 Improvements:
% - Automatically finds and loads the new `cycles_{timestamp}.csv` file.
% - `generate_h_matrix` function now simulates random apodization.
% - The slow ADMM reconstruction is commented out by default to allow for
%   fast H-matrix analysis.
% =========================================================================

clear; clc; close all;

%% ===== 1. CONFIGURATION AND FILE SELECTION =====
fprintf('=== pMUT REAL DATA RECONSTRUCTION (v2.16) ===\n');
...
cycles_file = ['cycles_' timestamp_str '.csv']; % NEW
...
cycles_filepath = fullfile(h5_path, cycles_file); % NEW
...
all_cycles = double(readmatrix(cycles_filepath, 'NumHeaderLines', 1)); % NEW
...
params.exp_cycles = all_cycles(1:params.num_acquisitions, :); % NEW
...
function [H, imaging_grid] = generate_h_matrix_from_profiles(config)
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        cycles = config.exp_cycles(r_acq, :); % NEW
        
        tx_signals = cell(1, num_active_tx); max_len = 0;
        for k = 1:num_active_tx
            duration = cycles(k) / tx_frequencies(k); % Duration is now based on cycle count
            t = 0:1/fs:duration;
            phase_rad = phases_deg(k) * pi / 180;
            % *** NEW: Modulate amplitude to simulate apodization ***
            % We use a simple linear scaling of the energy.
