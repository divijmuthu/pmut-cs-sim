% =========================================================================
% UNIFIED PMUT ULTRASOUND SIMULATION SCRIPT (v2.3 - Digital Twin w/ Filtering)
%
% Description:
% This version adds a critical signal processing step: a 70 kHz lowpass
% filter is applied to the raw experimental ADC data before reconstruction
% to remove out-of-band noise.
%
% v2.3 Changes:
% - Added a 70 kHz lowpass filter design.
% - Implemented a filtering loop to clean the raw ADC data before it is
%   passed to the ADMM reconstruction algorithm.
% =========================================================================
clear; clc; close all;
...
params.use_phase_shifts = true;    % Keep phase shifts enabled as it's our best model
...
tic;
% --- NEW: Design a 70 kHz lowpass filter ---
fprintf('  Designing 70 kHz lowpass filter...\n');
lp_filter = designfilt('lowpassfir', 'PassbandFrequency', 70e3, ...
                       'StopbandFrequency', 80e3, 'PassbandRipple', 0.5, ...
                       'StopbandAttenuation', 65, 'SampleRate', params.fs);

% --- NEW: Apply filter to each acquisition before assembling ---
filtered_adc_data = zeros(size(adc_data), 'double');
fprintf('  Applying filter to %d acquisitions...\n', params.num_acquisitions);
for i = 1:params.num_acquisitions
    filtered_adc_data(:, i) = filter(lp_filter, double(adc_data(:, i)));
end

b_vector = filtered_adc_data(:);
fprintf('Filtering and vector preparation complete. Time: %.2f seconds.\n', toc);
...
%% ===== HELPER FUNCTIONS =====
% (All helper functions remain the same as v2.2)
...
function [H, imaging_grid] = generate_h_matrix(config, delays_us, frequencies_hz)
    ...
    estimated_nnz = round(sum(cellfun(@(x) numel(x), all_h_data)) * 0.1);
