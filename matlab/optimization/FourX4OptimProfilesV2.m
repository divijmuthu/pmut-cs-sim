% =========================================================================
% UNIFIED PMUT ULTRASOUND SIMULATION SCRIPT (v3.1 - Final Corrected)
%
% Description:
% This version corrects errors from v3.0 by restoring the missing "Target
% Scene" parameter block and the full-featured `create_scene` helper
% function. This is the definitive, all-in-one script for generating
% optimal profiles and running the final imaging simulation.
% =========================================================================
clear; clc; close all;
%% ===== 1. MAIN SIMULATION CONFIGURATION =====
fprintf('=== pMUT SIMULATION (v3.1 - Final Corrected) ===\n');
...
main_output_folder = fullfile('integrated_greedy_output', timestamp);
...
% --- Target Scene Sweep Parameters ---  <-- THIS BLOCK WAS MISSING
...
for i = 1:length(target_patterns_to_test)
    ...
    fprintf('\n============================================================\n');
    fprintf('--- Running Simulation for Scene: "%s" ---\n', params.target_pattern);
    fprintf('============================================================\n');
...
function H = assemble_h_matrix_chunked(all_h_data, all_start_times, all_K_values, N_pixels, fs, chunk_size)
    ...
    for c_idx = 1:num_chunks
        ...
        % Pre-allocate for triplets
        est_nnz_per_row = 50; % Heuristic
...
function reconstructed_image = run_admm_reconstruction(H, b_vector, scene_matrix, config)
    ...
    wb_admm = waitbar(0, 'Running TV-ADMM...');
    for k_admm = 1:config.admm_max_iter
        ...
        if k_admm > 1 && (norm(x_admm_img_iter(:) - x_prev(:)) / (norm(x_prev(:)) + eps) < config.admm_tol), break; end
