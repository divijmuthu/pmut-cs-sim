% =========================================================================
% UNIFIED PMUT ULTRASOUND SIMULATION SCRIPT (v1.6 - Digital Twin)
%
% Description:
% This version adapts the simulation to be a "digital twin" of the real-world
% experimental setup. It uses hardware-constrained parameters for the
% transducers, delays, and signal generation.
%
% v1.6 Improvements:
% - Models a fixed 3-transducer triangular array, matching the physical setup.
% - Implements the specified 200us maximum delay.
% - Enforces the 8.67us discrete hardware delay ticks (quantization).
% - Disables random phase shifts and apodization to match experimental data.
% =========================================================================
clear; clc; close all;

%% ===== 1. MAIN SIMULATION CONFIGURATION =====
fprintf('=== UNIFIED PMUT ULTRASOUND SIMULATION (SCENE SWEEP) ===\n');
...
params.kerf_m = 0.001;             % Kerf between elements in virtual grid
...
% --- H-Matrix Generation Parameters (Based on REAL DATA) ---
params.num_acquisitions = 150;
params.num_active_tx = 3;          % HARDWARE CONSTRAINT: 3 transducers
params.max_delay_rand_us = 200;    % HARDWARE CONSTRAINT: 200us max delay
params.delay_tick_us = 8.67;       % HARDWARE CONSTRAINT: 8.67us delay resolution
params.apodization_mode = 'uniform'; % REAL DATA: No random apodization
...
%% ===== 2. GENERATE H-MATRIX (ONCE) =====
fprintf('\n--- Generating H-Matrix (based on real hardware constraints) ---\n');
tic;
...
function [H, imaging_grid] = generate_h_matrix(config)
    ...
    % --- Define a 9x9 virtual grid to place the fixed transducers ---
    ...
    % --- Define the 3 fixed transmitter positions ---
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % --- REALISTIC DELAYS: Quantized to hardware ticks ---
        num_ticks = floor(config.max_delay_rand_us / config.delay_tick_us);
        random_ticks = randi(num_ticks + 1, 1, num_active_tx) - 1;
        delays_us = random_ticks * config.delay_tick_us;
...
function scene_matrix = create_scene(imaging_grid, params)
    ...
    if length(params.target_sizes_mm) < num_targets
        warning('More targets than sizes defined. Reusing sizes.');
        params.target_sizes_mm = repmat(params.target_sizes_mm, 1, ceil(num_targets/length(params.target_sizes_mm)));
    end
    if length(params.target_intensities) < num_targets
         warning('More targets than intensities defined. Reusing intensities.');
        params.target_intensities = repmat(params.target_intensities, 1, ceil(num_targets/length(params.target_intensities)));
    end
