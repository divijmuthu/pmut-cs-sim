% =========================================================================
% FINAL REAL DATA RECONSTRUCTION SCRIPT (v3.2 - Numerically Stable)
%
% Description:
% This definitive version fixes a critical numerical scaling bug in the
% forward operator implementation. It now correctly normalizes both the
% sensing operator (H) and the measurement vector (b), ensuring the ADMM
% solver converges correctly. This combines the memory-efficiency of the
% forward operator with the numerical stability of the successful v1.5 sim.
%
% v3.2 Improvements:
% - Implemented correct normalization for both H and b.
% - The script should now converge correctly over many iterations.
% =========================================================================

clear; clc; close all;

%% ===== 1. CONFIGURATION AND FILE SELECTION =====
fprintf('=== pMUT FINAL RECONSTRUCTION (v3.2) ===\n');
...
function [all_h_aligned, imaging_grid, h_norm_factor] = generate_h_matrix_components(config)
    ...
    for r_acq = 1:config.num_acquisitions
        if all_K_values(r_acq) > 0 && ~isempty(all_h_data{r_acq})
            ...
            % Keep track of the max value for normalization
...
function reconstructed_image = run_admm_reconstruction(h_components, b_vector, scene_matrix_size, h_norm_factor, config)
    ...
    % --- Create Forward Operator Function Handles with Normalization ---
...
function [Afun, Atfun, AtAfun] = operator_setup(h_components, imageResolution, h_norm_factor, config)
    ...
    function y = forward_op(x_img)
        ...
        for i = 1:num_acqs
            ...
            % Apply normalization during the operation
