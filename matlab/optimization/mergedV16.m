% =========================================================================
% REAL DATA RECONSTRUCTION SCRIPT (v2.9 - Data Type Casting Fix)
%
% Description:
% This version fixes a data type conflict error that occurs because MATLAB
% automatically infers integer types when loading the experimental profiles.
%
% v2.9 Improvements:
% - Explicitly casts the `timebase` attribute and the data read from all
%   CSV files to `double`. This prevents type mismatch errors during
%   H-matrix simulation calculations.
% =========================================================================

clear; clc; close all;

%% ===== 1. CONFIGURATION AND FILE SELECTION =====
fprintf('=== pMUT REAL DATA RECONSTRUCTION (v2.9) ===\n');
...
% *** FIX: Cast timebase value to double to prevent type errors ***
...
% --- Transpose data to correct for Python/MATLAB dimension order mismatch ---
...
% *** FIX: Cast all loaded profile data to double ***
params.exp_frequencies_hz = double(readmatrix(freq_filepath, 'NumHeaderLines', 1));
params.exp_delays_us = double(readmatrix(delay_filepath, 'NumHeaderLines', 1));
params.exp_phases_deg = double(readmatrix(phase_filepath, 'NumHeaderLines', 1));
...
%% ===== HELPER FUNCTIONS (Identical to v2.8, included for completeness) =====
