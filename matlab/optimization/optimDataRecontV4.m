% =========================================================================
% FINAL REAL DATA RECONSTRUCTION SCRIPT (v3.5 - Numerical Scaling Fix)
%
% Description:
% This definitive version fixes a critical numerical scaling bug that caused
% the ADMM regularization to have no effect. By significantly increasing
% the `rho_admm` parameter, we re-balance the data and regularization terms,
% allowing the algorithm to correctly suppress noise and artifacts in the
% real-world data.
%
% v3.5 Improvements:
% - Corrected ADMM hyperparameters (`rho_admm`, `lambda_tv_reg`) to handle
%   the scale mismatch between simulated H and real-world b.
% =========================================================================

clear; clc; close all;

%% ===== 1. CONFIGURATION AND FILE SELECTION =====
fprintf('=== pMUT FINAL RECONSTRUCTION (v3.5) ===\n');
...
% These settings are tuned to handle the scale mismatch between the
% simulated H-operator and the real-world measurement vector b.
%
% rho_admm: The "Penalty" parameter.
% A much larger value is needed to amplify the "clean-up" term to a level
% where it can compete with the powerful, real-world data term.
params.rho_admm = 500.0; % (Previous: 5.0)

% lambda_tv_reg: The "Clean-Up" / "Denoising" parameter.
% Now that rho is correctly scaled, this can be a more standard, sensitive value.
params.lambda_tv_reg = 2.0; % (Previous: 150.0)

% admm_max_iter: The number of iterations.
