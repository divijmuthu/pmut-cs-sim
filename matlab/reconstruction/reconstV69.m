% =========================================================================
% FINAL REAL DATA RECONSTRUCTION SCRIPT (v4.5 - Corrected Geometry)
%
% Description:
% This version incorporates the precise physical geometry of the 3-pMUT
% triangular array, using the experimentally measured 4.7 cm center-to-
% center distance. This improves the accuracy of the "digital twin" model.
%
% v4.5 Improvements:
% - Updated `tx_desired_positions` to match the 4.7 cm equilateral
%   triangle geometry of the physical hardware.
% =========================================================================

clear; clc; close all;

%% ===== 1. CONFIGURATION AND FILE SELECTION =====
fprintf('=== pMUT FINAL RECONSTRUCTION (v4.5) ===\n');
...
params.c = 343;
params.pmut_width_m = 0.002; % Corrected 2mm pMUT width
params.kerf_m = 0.018;       % Corrected 18mm kerf for 20mm pitch
...
function [all_h_aligned, imaging_grid, h_norm_factor] = generate_h_matrix_components(config)
    ...
    % *** FIX: Define positions for a 4.7cm equilateral triangle ***
    s = 0.047; % 4.7 cm side length
    tx_desired_positions = [ 0, (sqrt(3)/3)*s, 0; ...
                            -s/2, -(sqrt(3)/6)*s, 0; ...
                             s/2, -(sqrt(3)/6)*s, 0 ];
