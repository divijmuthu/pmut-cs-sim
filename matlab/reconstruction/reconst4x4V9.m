% =========================================================================
% EXPERIMENTAL DATA RECONSTRUCTION SCRIPT v3.2 (Final Integrated)
%
% Description:
% This script incorporates the fix for the xdc_excitation vector orientation
% and reverts to manual, one-by-one HDF5 file selection for direct user
% control.
%
% v3.2 Improvements:
% - Correctly transposes the excitation waveform to a row vector for the
%   `xdc_excitation` function, fixing the crash.
% - Reverted HDF5 file loading to two separate dialogs as requested.
% =========================================================================
clear; clc; close all;

%% ===== 1. CONFIGURATION AND FILE SELECTION =====
fprintf('=== Experimental Data Reconstruction Script (v3.2) ===\n');
...
function [H, imaging_grid] = generate_h_matrix(config, profiles, tx_profiles_pins, base_waveform, base_fs)
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % Transpose to a row vector for Field II, as required by the function
        xdc_excitation(TxAperture, base_waveform_resampled'); % <--- ERROR FIXED HERE
