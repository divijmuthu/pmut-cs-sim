%% Final Optimized TheoryTest2 Reconstruction
% Use optimized parameters for proper ADMM convergence
...
%% ===== CONFIGURATION =====
fprintf('=== FINAL OPTIMIZED THEORYTEST2 RECONSTRUCTION ===\n');

% Create output directory
timestamp = datestr(now, 'mmddyy_HHMMSS');
output_folder = sprintf('final_optimized_output/%s', timestamp);
if ~exist(output_folder, 'dir')
    mkdir(output_folder);
end
fprintf('Saving results to: %s\n', output_folder);

% Use OPTIMIZED parameters for proper convergence
params = struct();
params.fs = 100e6;  % 100 MHz sampling
params.c = 343;     % Speed of sound
params.excitation_amplitude = 1e8;
params.R_acquisitions = 10;  % Balanced acquisitions
params.grid_width_mm = 80;   % Balanced grid width
params.grid_step_mm = 4;     % Balanced grid step
params.target_distance_m = 0.05;  % Target distance

fprintf('Configuration: OPTIMIZED TheoryTest2 parameters\n');
...
[target_scene, imaging_grid] = create_optimized_target_scene(params);

%% ===== STEP 2: GENERATE H MATRIX USING OPTIMIZED APPROACH =====
fprintf('\n=== STEP 2: GENERATING H MATRIX (OPTIMIZED) ===\n');
[H_matrix, reconstruction_info] = generate_optimized_h_matrix(params, imaging_grid);

%% ===== STEP 3: SIMULATE MEASUREMENTS =====
fprintf('\n=== STEP 3: SIMULATING MEASUREMENTS ===\n');
[measurements, measurement_times] = simulate_optimized_measurements(target_scene, H_matrix);

%% ===== STEP 4: OPTIMIZED ADMM RECONSTRUCTION =====
fprintf('\n=== STEP 4: OPTIMIZED ADMM RECONSTRUCTION ===\n');

% Use OPTIMIZED ADMM parameters for convergence
lambda_tv = 0.05;  % Reduced from 0.1 for better convergence
rho = 2.0;         % Increased from 1.0 for better convergence
max_iter = 500;    % Increased from 100 for convergence
tol = 1e-4;        % Relaxed from 1e-6 for practical convergence

fprintf('  ADMM parameters (OPTIMIZED):\n');
fprintf('    - lambda_tv: %.2f (reduced for convergence)\n', lambda_tv);
fprintf('    - rho: %.1f (increased for convergence)\n', rho);
fprintf('    - max_iter: %d (increased for convergence)\n', max_iter);
fprintf('    - tol: %.1e (relaxed for practical convergence)\n', tol);

% Perform optimized ADMM reconstruction
[reconstructed_image, reconstruction_time, admm_info] = ...
    perform_optimized_admm_reconstruction(measurements, H_matrix, lambda_tv, rho, max_iter, tol, imaging_grid);

%% ===== STEP 5: CALCULATE METRICS =====
fprintf('\n=== STEP 5: CALCULATING METRICS ===\n');
metrics = calculate_optimized_metrics(target_scene, reconstructed_image);

%% ===== STEP 6: SAVE RESULTS =====
fprintf('\n=== STEP 6: SAVING RESULTS ===\n');
save(fullfile(output_folder, 'optimized_results.mat'), ...
...
sgtitle(sprintf('Optimized TheoryTest2 Results - PSNR: %.2f dB, Corr: %.4f', ...
    metrics.psnr, metrics.correlation), 'FontSize', 14);

saveas(gcf, fullfile(output_folder, 'optimized_reconstruction.png'));
...
function [target_scene, imaging_grid] = create_optimized_target_scene(params)
    % Create target scene using optimized approach
    ...
    % Create targets (optimized style)
    target_positions = [
...
function [H_matrix, reconstruction_info] = generate_optimized_h_matrix(params, imaging_grid)
    % Generate H matrix using OPTIMIZED approach
    
    % Initialize Field II
    fprintf('  Initializing Field II...\n');
    field_init(-1);
    
    % Setup transducers (OPTIMIZED)
    pMUT_width = 0.002;  % 2mm pMUT
    pMUT_height = pMUT_width;
    kerf = 0.0005;  % 0.5mm kerf
    
    % Create 2D array apertures (OPTIMIZED)
    num_x_grid = 9;
    num_y_grid = 9;
    ...
    % Enable specific elements (OPTIMIZED)
    ...
    % Create apertures using xdc_2d_array (OPTIMIZED)
    ...
    % H matrix generation with OPTIMIZED approach
    ...
    fprintf('  Starting %d acquisitions (OPTIMIZED)...\n', params.R_acquisitions);
    
    for r_acq = 1:params.R_acquisitions
        % Generate excitation signals (OPTIMIZED)
        f_min = 45e3;
        f_max = 65e3;
    ...
    % OPTIMIZED time-domain interpolation
    fprintf('  Performing time-domain interpolation (OPTIMIZED)...\n');
    ...
    % Create common time axis (OPTIMIZED - balanced time samples)
    t_common_axis = min_global_start_time : 2/params.fs : max_global_end_time;
    ...
    for r_acq = 1:params.R_acquisitions
        ...
        % Time interpolation (OPTIMIZED)
        t_current_acq_axis = start_time_current + (0:(K_current - 1)) / params.fs;
        hhp_aligned_r = optimized_interpolation(t_current_acq_axis, hhp_current, t_common_axis, K_global_per_acq, imaging_grid.N_pixels);
    ...
    fprintf('  H matrix generation completed (OPTIMIZED)\n');
    fprintf('  - Matrix dimensions: %d x %d\n', size(H_matrix, 1), size(H_matrix, 2));
    fprintf('  - Sparsity: %.2f%%\n', 100 * (1 - nnz(H_matrix) / numel(H_matrix)));
end

function h_interpolated = optimized_interpolation(t_current_acq_axis, h_current, t_common_axis, K_global_per_acq, N_pixels)
...
function [measurements, measurement_times] = simulate_optimized_measurements(target_scene, H_matrix)
    % Simulate measurements using optimized approach
...
function [reconstructed_image, reconstruction_time, admm_info] = perform_optimized_admm_reconstruction(measurements, H_matrix, lambda_tv, rho, max_iter, tol, imaging_grid)
    % Perform ADMM reconstruction using OPTIMIZED approach
    ...
    fprintf('    Starting optimized ADMM reconstruction...\n');
    ...
    for iter = 1:max_iter
        ...
        z_prev = z;
        z = optimized_prox_tv(x + u, lambda_tv / rho, num_x, num_z);
        ...
        tv_term = lambda_tv * optimized_tv_norm(x, num_x, num_z);
        objective_values(iter) = data_term + tv_term;
        
        % Check convergence (OPTIMIZED criteria)
        if primal_residual < tol && dual_residual < tol
            fprintf('    Converged at iteration %d\n', iter);
            break;
        end
        
        % Print progress every 50 iterations (reduced for longer runs)
        if mod(iter, 50) == 0
    ...
    fprintf('    Optimized ADMM completed in %.2f seconds\n', reconstruction_time);
    fprintf('    - Iterations: %d\n', iter);
    fprintf('    - Converged: %s\n', mat2str(admm_info.converged));
    fprintf('    - Final primal residual: %.2e\n', primal_residual);
end

function z = optimized_prox_tv(x, lambda, num_x, num_z)
    % Optimized TV proximal operator
    ...
    % Gradient descent for TV proximal operator (OPTIMIZED)
    max_inner_iter = 100;  % Increased for better convergence
    inner_tol = 1e-8;      % Tighter tolerance
...
function tv_val = optimized_tv_norm(x, num_x, num_z)
    % Calculate TV norm for optimized approach
...
function metrics = calculate_optimized_metrics(target_scene, reconstructed_image)
...
fprintf('\n=== FINAL OPTIMIZED THEORYTEST2 RECONSTRUCTION COMPLETE ===\n');
