%% Balanced TheoryTest2 Reconstruction
% Use balanced parameters to get proper H matrix conditioning
...
%% ===== CONFIGURATION =====
fprintf('=== BALANCED THEORYTEST2 RECONSTRUCTION ===\n');

% Create output directory
timestamp = datestr(now, 'mmddyy_HHMMSS');
output_folder = sprintf('balanced_theorytest2_output/%s', timestamp);
if ~exist(output_folder, 'dir')
    mkdir(output_folder);
end
fprintf('Saving results to: %s\n', output_folder);

% Use BALANCED parameters for proper conditioning
params = struct();
params.fs = 100e6;  % 100 MHz sampling
params.c = 343;     % Speed of sound
params.excitation_amplitude = 1e8;
params.R_acquisitions = 10;  % Reduced acquisitions (was 20)
params.grid_width_mm = 80;   % Larger grid width (was 50)
params.grid_step_mm = 4;     % Smaller grid step (was 5)
params.target_distance_m = 0.05;  % Target distance

fprintf('Configuration: BALANCED TheoryTest2 parameters\n');
...
[target_scene, imaging_grid] = create_balanced_target_scene(params);

%% ===== STEP 2: GENERATE H MATRIX USING BALANCED APPROACH =====
fprintf('\n=== STEP 2: GENERATING H MATRIX (BALANCED) ===\n');
[H_matrix, reconstruction_info] = generate_balanced_h_matrix(params, imaging_grid);

%% ===== STEP 3: SIMULATE MEASUREMENTS =====
fprintf('\n=== STEP 3: SIMULATING MEASUREMENTS ===\n');
[measurements, measurement_times] = simulate_balanced_measurements(target_scene, H_matrix);

%% ===== STEP 4: BALANCED ADMM RECONSTRUCTION =====
fprintf('\n=== STEP 4: BALANCED ADMM RECONSTRUCTION ===\n');

% Use BALANCED ADMM parameters
lambda_tv = 0.1;  % From TheoryTest2
rho = 1.0;        % From TheoryTest2
max_iter = 100;   % From TheoryTest2
tol = 1e-6;       % From TheoryTest2

fprintf('  ADMM parameters (BALANCED):\n');
fprintf('    - lambda_tv: %.2f\n', lambda_tv);
fprintf('    - rho: %.1f\n', rho);
fprintf('    - max_iter: %d\n', max_iter);
fprintf('    - tol: %.1e\n', tol);

% Perform balanced ADMM reconstruction
[reconstructed_image, reconstruction_time, admm_info] = ...
    perform_balanced_admm_reconstruction(measurements, H_matrix, lambda_tv, rho, max_iter, tol, imaging_grid);

%% ===== STEP 5: CALCULATE METRICS =====
fprintf('\n=== STEP 5: CALCULATING METRICS ===\n');
metrics = calculate_balanced_metrics(target_scene, reconstructed_image);

%% ===== STEP 6: SAVE RESULTS =====
fprintf('\n=== STEP 6: SAVING RESULTS ===\n');
save(fullfile(output_folder, 'balanced_results.mat'), ...
...
sgtitle(sprintf('Balanced TheoryTest2 Results - PSNR: %.2f dB, Corr: %.4f', ...
    metrics.psnr, metrics.correlation), 'FontSize', 14);

saveas(gcf, fullfile(output_folder, 'balanced_reconstruction.png'));
...
function [target_scene, imaging_grid] = create_balanced_target_scene(params)
    % Create target scene using balanced approach
    
    % Grid parameters (BALANCED)
...
function [H_matrix, reconstruction_info] = generate_balanced_h_matrix(params, imaging_grid)
    % Generate H matrix using BALANCED approach
    
    % Initialize Field II
    fprintf('  Initializing Field II...\n');
    field_init(-1);
    
    % Setup transducers (BALANCED)
    pMUT_width = 0.002;  % 2mm pMUT
    pMUT_height = pMUT_width;
    kerf = 0.0005;  % 0.5mm kerf
    
    % Create 2D array apertures (BALANCED)
    num_x_grid = 9;
    num_y_grid = 9;
    ...
    % Enable specific elements (BALANCED)
    ...
    % Create apertures using xdc_2d_array (BALANCED)
    ...
    % H matrix generation with BALANCED approach
    ...
    fprintf('  Starting %d acquisitions (BALANCED)...\n', params.R_acquisitions);
    
    for r_acq = 1:params.R_acquisitions
        % Generate excitation signals (BALANCED)
        f_min = 45e3;
        f_max = 65e3;
    ...
    % BALANCED time-domain interpolation
    fprintf('  Performing time-domain interpolation (BALANCED)...\n');
    ...
    % Create common time axis (BALANCED - fewer time samples)
    t_common_axis = min_global_start_time : 2/params.fs : max_global_end_time;  % Reduced time samples
    ...
    for r_acq = 1:params.R_acquisitions
        ...
        % Time interpolation (BALANCED)
        t_current_acq_axis = start_time_current + (0:(K_current - 1)) / params.fs;
        hhp_aligned_r = balanced_interpolation(t_current_acq_axis, hhp_current, t_common_axis, K_global_per_acq, imaging_grid.N_pixels);
    ...
    fprintf('  H matrix generation completed (BALANCED)\n');
    fprintf('  - Matrix dimensions: %d x %d\n', size(H_matrix, 1), size(H_matrix, 2));
    fprintf('  - Sparsity: %.2f%%\n', 100 * (1 - nnz(H_matrix) / numel(H_matrix)));
end

function h_interpolated = balanced_interpolation(t_current_acq_axis, h_current, t_common_axis, K_global_per_acq, N_pixels)
    % BALANCED interpolation function
...
function [measurements, measurement_times] = simulate_balanced_measurements(target_scene, H_matrix)
    % Simulate measurements using balanced approach
...
function [reconstructed_image, reconstruction_time, admm_info] = perform_balanced_admm_reconstruction(measurements, H_matrix, lambda_tv, rho, max_iter, tol, imaging_grid)
    % Perform ADMM reconstruction using BALANCED approach
    ...
    fprintf('    Starting balanced ADMM reconstruction...\n');
    ...
    for iter = 1:max_iter
        ...
        z_prev = z;
        z = balanced_prox_tv(x + u, lambda_tv / rho, num_x, num_z);
        ...
        tv_term = lambda_tv * balanced_tv_norm(x, num_x, num_z);
    ...
    fprintf('    Balanced ADMM completed in %.2f seconds\n', reconstruction_time);
    fprintf('    - Iterations: %d\n', iter);
    fprintf('    - Converged: %s\n', mat2str(admm_info.converged));
    fprintf('    - Final primal residual: %.2e\n', primal_residual);
end

function z = balanced_prox_tv(x, lambda, num_x, num_z)
    % Balanced TV proximal operator
...
function tv_val = balanced_tv_norm(x, num_x, num_z)
    % Calculate TV norm for balanced approach
...
function metrics = calculate_balanced_metrics(target_scene, reconstructed_image)
    % Calculate metrics using balanced approach
...
fprintf('\n=== BALANCED THEORYTEST2 RECONSTRUCTION COMPLETE ===\n');
