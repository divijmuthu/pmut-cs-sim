% =========================================================================
% Ultrasound Reconstruction Script v2.3 (Alignment & ADMM Fix)
%
% Description:
% This version fixes two critical bugs:
% 1. A data-type crash in the ADMM solver.
% 2. A time-axis alignment error that caused poor reconstruction quality.
% It also standardizes the color scale on the final plots for easier
% comparison.
% =========================================================================
clearvars;
clc;
...
echo_data_truncated = echo_data_matrix_filtered(:, 1:samples_to_keep_raw);
fprintf('Data truncated to first %d ms (%d samples).\n', DATA_TRUNCATE_MS, samples_to_keep_raw);
clear echo_data_matrix_filtered;

% --- AGGRESSIVE DOWNSAMPLING ---
fs_new = fs_raw / DOWNSAMPLING_FACTOR;
echo_data_downsampled = resample(echo_data_truncated', 1, DOWNSAMPLING_FACTOR)';
...
% *** FIX: Correctly calculate time axis with pre-trigger offset ***
pre_trigger_samples_new = round(total_samples_new * ( (0.1 * total_samples_raw) / total_samples_raw) ); % Maintain 10% pre-trigger ratio
time_axis_new = ((0:total_samples_new-1) - pre_trigger_samples_new) / fs_new;
...
x_pinv = pinv(A_TA) * A_Tb; % More stable way to calculate pinv
fprintf('Done.\n');
...
% *** NEW: Standardize color scale for better comparison ***
...
function [Grad, Div] = get_div_grad_operators(n, m)
    ...
    D_x(n,n) = 0; D_y(m,m) = 0;
    % *** FIX: Cast input to double to prevent type mismatch error ***
