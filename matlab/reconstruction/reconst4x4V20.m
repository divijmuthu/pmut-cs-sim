% =========================================================================
% EXPERIMENTAL DATA RECONSTRUCTION SCRIPT (v6.0 - Freq/Delay Only)
%
% Description:
% This is the definitive script for reconstructing the experimental data,
% aligned with a data collection process that uses only frequency and
% delay for signal diversity (no phase or apodization).
% =========================================================================
clear; clc; close all;

%% ===== 1. CONFIGURATION =====
fprintf('=== Experimental Data Reconstruction Script (v6.0) ===\n');
...
non_rx_grid_pos = [1, 2, 4, 5, 6, 7, 8, 9, 12, 13, 15, 16];
hardware_pins   = [26, 24, 28, 29, 30, 31, 32, 33, 34, 36, 37, 39]; % Match Arduino
...
params.kerf_m = 0.005;
params.excitation_amplitude = 1e12; % Using normalized amplitude
...
params.num_acquisitions = size(tx_pin_profiles, 1);
...
original_fs = 1 / ( (4-2)/62.5e6 ); % From Python TIMEBASE=4
...
%% ===== 5. ANALYZE H-MATRIX COHERENCE =====
fprintf('\n--- Step 4: Analyzing H-Matrix Coherence ---\n');
...
function [H, imaging_grid] = generate_h_matrix(config, profiles, tx_profiles_pins)
    ...
    pin_to_grid_map = values(config.GRID_TO_PIN_MAP);
    pin_to_grid_map = [pin_to_grid_map{:}];
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        [~, grid_indices_for_pins] = ismember(active_tx_pins, pin_to_grid_map);
        active_grid_indices = config.GRID_TO_PIN_MAP.keys;
        active_grid_indices = [active_grid_indices{:}];
        active_grid_indices = active_grid_indices(grid_indices_for_pins);
        ...
        % --- Simplified Waveform Synthesis (Frequency and Delay ONLY) ---
        max_len = 0;
        tx_signals = cell(1, config.num_active_tx);
        for k = 1:config.num_active_tx
            f_k = tx_frequencies(k);
            d_k = delays_us(k) * 1e-6; % Convert delay to seconds
            duration = 7 / f_k; % 7 cycles
            t_k = d_k : 1/fs : (d_k + duration);
            signal_k = sin(2*pi*f_k*(t_k - d_k)); % Time-shifted sine wave
        ...
        xdc_excitation(TxAperture, composite_waveform * config.excitation_amplitude);
...
function H = assemble_h_matrix_chunked(all_h_data, all_start_times, all_K_values, N_pixels, fs, chunk_size)
    ...
    for c_idx = 1:num_chunks
        ...
        parfor r_acq_local = 1:(end_acq - start_acq + 1)
