% =========================================================================
% EXPERIMENTAL DATA RECONSTRUCTION SCRIPT (v5.7 - Synthesis Fix)
%
% Description:
% This version fixes a critical bug in the H-matrix waveform synthesis.
% It now uses a timeline-based approach that correctly models the
% complex overlaps between the 8 active transmitters, resolving the
% zero-coherence issue.
% =========================================================================
clear; clc; close all;

%% ===== 1. CONFIGURATION =====
fprintf('=== Experimental Data Reconstruction Script (v5.7) ===\n');
...
function [H, imaging_grid] = generate_h_matrix_digital_twin(config, profiles, tx_pin_profiles)
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        % --- CORRECTED: Timeline-based Waveform Synthesis ---
        
        % 1. Create a master list of all toggle events for all 8 channels
        event_list = []; % [time_in_samples, channel_index]
        max_end_time = 0;
        for i = 1:8
            delay_samples = round(delays_us(i) * (fs / 1e6));
            half_period_samples = round((fs / tx_frequencies_hz(i)) / 2);
            for j = 0:(BURST_CYCLES * 2 - 1)
                event_time = delay_samples + j * half_period_samples;
                event_list = [event_list; event_time, i];
            end
            if event_list(end, 1) > max_end_time
                max_end_time = event_list(end, 1);
            end
        end
        
        % 2. Sort the master list of events by time
        event_list = sortrows(event_list, 1);
        
        % 3. Build the composite waveform from the sorted timeline
        time_vector_len = max_end_time + 1;
        composite_wave = zeros(1, time_vector_len);
        channel_states = ones(1, 8); % Start positive
        
        last_time = 0;
        for i = 1:size(event_list, 1)
            current_time = event_list(i, 1);
            channel_to_toggle = event_list(i, 2);
            
            % Fill the waveform from the last event to this one
            if current_time > last_time
                current_state = sum(channel_states);
                composite_wave(last_time+1 : current_time) = current_state;
            end
            
            % Toggle the state of the current channel
            channel_states(channel_to_toggle) = -channel_states(channel_to_toggle);
            last_time = current_time;
        end
        % Fill the final segment
        composite_wave(last_time+1 : end) = sum(channel_states);
