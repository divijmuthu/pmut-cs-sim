% =========================================================================
% EXPERIMENTAL DATA RECONSTRUCTION SCRIPT v4.1 (True Digital Twin)
%
% Description:
% This version implements the final, critical fixes to create a true
% digital twin of the physical experiment. The H-matrix generation now
% simulates the digital square waves and single pre-delay used by the
% Python/Arduino system.
%
% v4.1 Improvements:
% - `generate_h_matrix` now correctly synthesizes digital square waves
%   based on the zero-crossings of the ideal analog pulse.
% - Correctly models the single average pre-delay per shot.
% - Removed redundant/incorrect Field II calls for a cleaner model.
% =========================================================================
clear; clc; close all;

%% ===== 1. CONFIGURATION =====
fprintf('=== Experimental Data Reconstruction Script (v4.1) ===\n');
...
% (This section is complete and remains unchanged)
...
function [H, imaging_grid] = generate_h_matrix_digital_twin(config, profiles, tx_profiles_pins)
    % This version creates a true digital twin by simulating the square
    % waves and single pre-delay of the physical experiment.
    fs = config.fs; c = config.c;
    field_init(-1); set_field('fs', fs); set_field('c', c);
    
    % --- Define geometry (same as before) ---
...
% (All other helper functions: analyze_coherence, etc. remain the same)
