% =========================================================================
% EXPERIMENTAL DATA RECONSTRUCTION SCRIPT v3.5 (Final w/ Optional Gating)
%
% Description:
% This is the definitive script for reconstructing the experimental data.
% It includes all previous bug fixes and incorporates an optional parameter
% to enable or disable the time-gating step.
%
% v3.5 Improvements:
% - Added `params.enable_time_gating` boolean flag to control time gating.
% - All previous fixes are integrated for a complete, runnable solution.
% =========================================================================
clear; clc; close all;

%% ===== 1. CONFIGURATION =====
fprintf('=== Experimental Data Reconstruction Script (v3.5) ===\n');
...
%% ===== 4. GENERATE, GATE, AND ANALYZE H-MATRIX =====
...
if params.enable_time_gating
    fprintf('\n--- Step 4: Applying Time Gate ---\n');
    % --- Diagnostic Check for Time Gate Alignment ---
    first_signal_sample = find(any(H_un_gated, 2), 1, 'first');
    last_signal_sample = find(any(H_un_gated, 2), 1, 'last');
    if ~isempty(first_signal_sample)
        actual_signal_start_time_s = (first_signal_sample / params.num_acquisitions - 1) / params.fs;
        actual_signal_end_time_s = (last_signal_sample / params.num_acquisitions - 1) / params.fs;
        fprintf('  INFO: Simulated signal data exists from %.4f ms to %.4f ms.\n', actual_signal_start_time_s * 1000, actual_signal_end_time_s * 1000);
    else
        fprintf('  WARNING: H_un_gated appears to be all zeros. Check simulation.\n');
    end
    gate_end_time_s = params.gate_start_time_s + params.gate_duration_s;
    fprintf('  INFO: Your current time gate is set from %.4f ms to %.4f ms.\n', params.gate_start_time_s * 1000, gate_end_time_s * 1000);
    if isempty(first_signal_sample) || gate_end_time_s < actual_signal_start_time_s || params.gate_start_time_s > actual_signal_end_time_s
        fprintf('  !! WARNING: Time gate does not overlap with signal data. H_final will be empty. Adjust "params.gate_start_time_s" !!\n');
    end
    ...
    fprintf('\n--- Step 4: Skipping Time Gate ---\n');
    H_final = H_un_gated;
    b_final = b_aligned(:);
end

fprintf('\n--- Step 5: Analyzing Final H-Matrix Coherence ---\n');
...
%% ===== 6. RECONSTRUCT IMAGE =====
fprintf('\n--- Step 6: Reconstructing Image via TV-ADMM ---\n');
tic;
...
fprintf('\n--- Step 7: Visualizing and Saving Results ---\n');
