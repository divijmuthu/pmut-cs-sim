% =========================================================================
% EXPERIMENTAL DATA RECONSTRUCTION SCRIPT (v1.8 - Final Pin Map)
%
% Description:
% This is the definitive script for reconstructing real experimental data.
% It has been updated with the user's specific, hand-drawn pin map to
% ensure a perfect sim-to-reality match.
%
% v1.8 Improvements:
% - Implemented the user-provided 4x4 hardware pin map and logic.
% - Correctly configured the disabled and receiver pMUTs based on this map.
% - Aligned all logic with the proven v3.2 simulation pipeline.
% =========================================================================
clear; clc; close all;
%% ===== 1. CONFIGURATION AND FILE SELECTION =====
fprintf('=== Experimental Data Reconstruction Script (v1.8) ===\n');
...
% =========================================================================
% --- CRITICAL: HARDWARE MAP (from user's drawing and description) ---
% This maps the GRID POSITION (1-16) to the ARDUINO HARDWARE PIN.
% The 14 non-receiving grid positions are mapped to pins 26-39.
...
% Specify GRID POSITIONS for disabled/receiver elements
...
tic;
[H, imaging_grid] = generate_h_matrix(params, profiles, tx_pin_profiles);
fprintf('H-Matrix generation complete. Time: %.2f seconds.\n', toc);

num_h_samples = size(H, 1) / params.num_acquisitions;
...
H_reshaped = reshape(full(H), [floor(num_h_samples), size(H, 2), params.num_acquisitions]);
H_aligned = H_reshaped(1:min_samples, :, :);
H_final = sparse(reshape(H_aligned, [min_samples * params.num_acquisitions, size(H, 2)]));
b_reshaped = reshape(b_vector_trimmed, [num_samples_per_acq_b, params.num_acquisitions]);
b_aligned = b_reshaped(1:min_samples, :);
b_final = b_aligned(:);
fprintf('  Aligned H and b to %d samples per acquisition.\n', min_samples);
...
function [H, imaging_grid] = generate_h_matrix(config, profiles, tx_profiles_pins)
    ...
    % --- Create a reverse map from Pin to Grid Position ---
