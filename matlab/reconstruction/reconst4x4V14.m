% =========================================================================
% EXPERIMENTAL DATA RECONSTRUCTION SCRIPT v4.0 (Waveform Synthesis)
%
% Description:
% This definitive version corrects the core "digital twin" mismatch. It
% abandons the flawed static waveform approach and correctly synthesizes a
% unique composite waveform for each acquisition, mirroring the logic of the
% original successful simulation and the physical experiment.
%
% v4.0 Improvements:
% - REMOVED loading of the .mat waveform files.
% - The `generate_h_matrix` function now programmatically synthesizes the
%   correct composite waveform for each of the 150 shots.
% - This version should finally produce a meaningful, non-zero coherence.
% =========================================================================
clear; clc; close all;

%% ===== 1. CONFIGURATION =====
fprintf('=== Experimental Data Reconstruction Script (v4.0) ===\n');
...
params.enable_time_gating = true; % SET TO true (recommended) or false
params.gate_start_time_s = 0.0007;  % Adjust this based on diagnostic output
...
% --- NOTE: We no longer load the .mat waveform files ---

% --- Load main experimental data from HDF5 files ---
...
tic;
[H_raw, imaging_grid] = generate_h_matrix_synthesized(params, profiles, tx_pin_profiles);
...
if params.enable_time_gating
    fprintf('  Applying time gate...\n');
...
function [H, imaging_grid] = generate_h_matrix_synthesized(config, profiles, tx_profiles_pins)
    % This new version synthesizes the composite waveform for each shot,
    % matching the logic from the working simulation script.
    ...
    NUM_MAIN_CYCLES = 7; % Match the Python script
    NUM_CANCEL_CYCLES = 1; % Match the Python script
    
    wb = waitbar(0, 'Generating H matrix with synthesized waveforms...');
    for r_acq = 1:config.num_acquisitions
        ...
        % --- Synthesize the unique composite waveform for this acquisition ---
        ...
        max_len_main = 0;
        signals_main = cell(1, config.num_active_tx);
        for k = 1:config.num_active_tx
            duration = NUM_MAIN_CYCLES / tx_frequencies(k);
            t_k = 0:1/fs:duration;
            % Using a tukey window to approximate the Blackman from Python
            signal_k = apod_weights(k) * sin(2*pi*tx_frequencies(k)*t_k + tx_phases(k)) .* tukeywin(length(t_k), 0.5)';
        ...
        for k = 1:config.num_active_tx
            sig = signals_main{k};
            composite_main(1:length(sig)) = composite_main(1:length(sig)) + sig;
        end
        
        % Cancellation Pulse Synthesis
        max_len_cancel = 0;
        signals_cancel = cell(1, config.num_active_tx);
        for k = 1:config.num_active_tx
            duration = NUM_CANCEL_CYCLES / tx_frequencies(k);
            t_k = 0:1/fs:duration;
            signal_k = apod_weights(k) * sin(2*pi*tx_frequencies(k)*t_k + tx_phases(k) + pi) .* tukeywin(length(t_k), 0.5)';
        ...
        for k = 1:config.num_active_tx
            sig = signals_cancel{k};
            composite_cancel(1:length(sig)) = composite_cancel(1:length(sig)) + sig;
        end

        final_composite_waveform = [composite_main, composite_cancel];
        
        % Set the synthesized waveform as the impulse response/excitation
        xdc_impulse(TxAperture, final_composite_waveform * config.excitation_amplitude);
...
% (All other helper functions: analyze_coherence, load_and_average_waveform, etc. remain the same)
% ... Paste the other helper functions from the previous script version here ...
