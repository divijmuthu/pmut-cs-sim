% =========================================================================
% EXPERIMENTAL DATA RECONSTRUCTION SCRIPT (v2.0 - Robust Waveform Loading)
%
% Description:
% This is the definitive script for reconstructing real experimental data.
% It includes an integrated function to load and average the multiple
% .mat files exported from a PicoScope .psdata file.
%
% v2.0 Improvements:
% - FIXED a crash by making the waveform loader smarter. It now automatically
%   finds the longest numeric vector in the .mat files, correctly identifying
%   the waveform data.
% - Reverted to the correct v1.5 hardware pin map.
% =========================================================================
clear; clc; close all;
%% ===== 1. CONFIGURATION AND FILE SELECTION =====
fprintf('=== Experimental Data Reconstruction Script (v2.0) ===\n');
...
% --- Load and average the measured pMUT waveform ---
...
[base_waveform, base_waveform_fs] = load_and_average_waveform_folder(waveform_folder);
...
%% ===== HELPER FUNCTIONS =====
function [waveform, fs] = load_and_average_waveform_folder(folder_path)
    mat_files = dir(fullfile(folder_path, '*.mat'));
    if isempty(mat_files)
        error('No .mat files found in the selected folder.');
    end
    
    fprintf('  Found %d waveform segments. Averaging...\n', length(mat_files));
    
    first_file_data = load(fullfile(folder_path, mat_files(1).name));
    
    % --- NEW: Robustly find the waveform data by finding the longest vector ---
    fields = fieldnames(first_file_data);
    waveform_var_name = '';
    max_len = 0;
    for i = 1:length(fields)
        field_content = first_file_data.(fields{i});
        if isnumeric(field_content) && isvector(field_content) && length(field_content) > max_len
            waveform_var_name = fields{i};
            max_len = length(field_content);
        end
    end
    if isempty(waveform_var_name)
        error('Could not find a valid numeric vector in the first .mat file.');
    end
    fprintf('  Detected waveform data in variable: "%s"\n', waveform_var_name);
    
    fs = 1 / first_file_data.Tstart;
    num_samples = length(first_file_data.(waveform_var_name));
    accumulator = zeros(num_samples, 1);
    
    for i = 1:length(mat_files)
        data = load(fullfile(folder_path, mat_files(i).name));
        accumulator = accumulator + data.(waveform_var_name);
    end
...
function [H, imaging_grid] = generate_h_matrix(config, profiles, tx_profiles_pins, base_waveform, base_fs)
    ...
    base_waveform_resampled = resample(base_waveform, fs, base_fs);
    ...
    for r_acq = 1:config.num_acquisitions
        ...
        disabled_grid_indices = config.disabled_pmuts_grid;
        ...
        xdc_impulse(TxAperture, base_waveform_resampled);
