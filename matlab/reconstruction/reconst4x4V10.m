% =========================================================================
% EXPERIMENTAL DATA RECONSTRUCTION SCRIPT v3.3 (Final Cohesive)
%
% Description:
% This is the final, fully integrated script for reconstructing images.
% It incorporates all successful bug fixes and requested features, including
% a diagnostic coherence check and a progress bar for the ADMM solver.
%
% v3.3 Improvements:
% - All previous fixes are integrated.
% - Includes a diagnostic section to visualize H-matrix coherence.
% - Confirmed inclusion of the waitbar for ADMM reconstruction progress.
% =========================================================================
clear; clc; close all;

%% ===== 1. CONFIGURATION =====
fprintf('=== Experimental Data Reconstruction Script (v3.3) ===\n');
...
% --- DC Offset Removal and Channel Averaging ---
...
% --- Align Measured Data (b) with Simulated Matrix (H) ---
...
%% ===== 5. DIAGNOSTIC: H-MATRIX COHERENCE CHECK =====
fprintf('\n--- Step 4: Performing H-Matrix Coherence Check ---\n');
N_pixels = size(imaging_grid.points, 1);
num_gated_samples = size(b_gated_list{1}, 1);
px_center = floor(N_pixels / 2);
px_top = floor(size(imaging_grid.X_mesh, 2) / 2);
px_corner = 1;
pixel_indices_to_check = [px_center, px_top, px_corner];
figure('Name', 'H-Matrix Coherence Check', 'Position', [50, 50, 1200, 500]);
sgtitle('H-Matrix Coherence Check (Pixel Fingerprints)');
clim_max = 0;
for px_idx = pixel_indices_to_check
    h_col = full(H_final(:, px_idx));
    clim_max = max(clim_max, max(abs(h_col)));
end
for i = 1:length(pixel_indices_to_check)
    subplot(1, length(pixel_indices_to_check), i);
    h_col = full(H_final(:, pixel_indices_to_check(i)));
    h_reshaped = reshape(h_col, num_gated_samples, params.num_acquisitions);
    imagesc(h_reshaped);
    xlabel('Acquisition Index');
    ylabel('Gated Time Samples');
    title(sprintf('Pixel %d', pixel_indices_to_check(i)));
    colorbar;
    if clim_max > 0, caxis([-clim_max clim_max]); end
end
colormap('jet');
fprintf('  Displayed H-matrix columns for pixels %s.\n', num2str(pixel_indices_to_check));
fprintf('  Check the figure: patterns should look complex and non-repeating.\n');
...
function reconstructed_image = run_admm_reconstruction(H, b_vector, scene_matrix_size, config)
    ...
    wb_admm = waitbar(0, 'Running TV-ADMM Reconstruction...'); % Progress bar initiated
    for k_admm = 1:config.admm_max_iter
        ...
        if ishandle(wb_admm), waitbar(k_admm / config.admm_max_iter, wb_admm, sprintf('ADMM Iteration %d / %d', k_admm, config.admm_max_iter)); end % Progress bar updated
    end
    if ishandle(wb_admm), close(wb_admm); end % Progress bar closed
