...
for iy = 1:num_y_grid
    y_pos_el = (iy - 1) * (pMUT_height + kerf) - center_offset_y;
    for ix = 1:num_x_grid
        x_pos_el = (ix - 1) * (pMUT_width + kerf) - center_offset_x;
...
for i = 1:num_active_intended
    distances = sum((physical_element_centers - desired_positions(i, :)).^2, 2);
...
if num_active < num_active_intended
    warning('Could only map %d unique elements.', num_active);
end
if num_active == 0
...
for r_acq = 1:R_acquisitions
    ...
    if num_active == 0
        error('Cannot proceed with 0 active elements.');
    end
...
if isempty(min_global_start_time) || isempty(max_global_end_time) || min_global_start_time >= max_global_end_time
    ...
    if max_global_end_time < min_global_start_time
        max_global_end_time = min_global_start_time + (100 - 1) / fs;
    end
...
for n_idx_plot = 1:length(indices_to_plot_H)
    ...
    plot(t_axis_H_plot_full_concat, H(:, col_idx), 'DisplayName', sprintf('H col Px %d', col_idx));
end

hold off;
xlabel('Overall Row Index (us)');
ylabel('Amplitude');
title('Figure 3: Columns of Assembled H');
axis tight;
grid on;
...
scene_xmin = min(x_coords_img);
scene_xmax = max(x_coords_img);
scene_zmin = min(z_coords_img);
scene_zmax = max(z_coords_img);
scene_z_range = scene_zmax - scene_zmin;

targets.x = [scene_xmin * 0.3, 0, scene_xmax * 0.3];
targets.y = zeros(size(targets.x));
targets.z = [scene_zmin + 0.25 * scene_z_range, scene_zmin + 0.5 * scene_z_range, scene_zmin + 0.75 * scene_z_range];
...
xlabel('x (mm)');
ylabel('z (mm)');
title('True Image v (matrix)');
set(gca, 'YDir', 'normal');

subplot(1, 2, 2);
stem(v_true_vector);
axis square tight;
title('True Image v (vector)');
sgtitle('Figure 4: True Image');
...
if isnan(signal_power_est) || signal_power_est < 1e-30
    warning('Signal power zero/NaN (%g). Small fallback noise.', signal_power_est);
    noise_sigma = 1e-9 * max(abs(H(:)));
    if abs(noise_sigma) < eps
        noise_sigma = 1e-15;
    end
    actual_SNR_db = -Inf;
else
...
figure(6);
clf;
t_ax_meas = (0:(M - 1)) / fs * 1e6;
plot_lim_samp = min(M, round(fs * max_global_end_time * 1.1));
if plot_lim_samp == 0 && M > 0
    plot_lim_samp = M;
end

subplot(1, 3, 1);
if ~isempty(Hv_signal) && plot_lim_samp > 0
    plot(t_ax_meas(1:plot_lim_samp), Hv_signal(1:plot_lim_samp));
end
axis tight; grid on; xlabel('Time (us)'); title('Hv - Ideal'); yl = ylim;

subplot(1, 3, 2);
if ~isempty(n_noise_vec) && plot_lim_samp > 0
    plot(t_ax_meas(1:plot_lim_samp), n_noise_vec(1:plot_lim_samp));
end
axis tight; grid on; xlabel('Time (us)'); title(sprintf('Noise (SNR~%.1fdB)', actual_SNR_db));
if all(isfinite(yl)); ylim(yl); end

subplot(1, 3, 3);
if ~isempty(u_measured_signal) && plot_lim_samp > 0
    plot(t_ax_meas(1:plot_lim_samp), u_measured_signal(1:plot_lim_samp));
end
...
if use_pcg_regularization
    ...
    AAtfun_pcg_eff = @(y_vec) AAtfun_pcg(y_vec) + alpha_pcg_reg * y_vec;
end
...
if M == 0 || N == 0 || isempty(b_vector) || norm(b_vector(:)) == 0
    ...
    if flag_cg_main ~= 0
        warning('PCG did not converge as expected (Flag=%d).', flag_cg_main);
    end
...
min_v = min(x_pcg_proc(:));
max_v = max(x_pcg_proc(:));

if (max_v - min_v) > eps
    x_pcg_norm = (x_pcg_proc - min_v) / (max_v - min_v);
else
    fprintf('    DEBUG PCG: Raw dynamic range too small. Normalized image set to zeros.\n');
    x_pcg_norm = zeros(N, 1);
end
x_pcg_norm(isnan(x_pcg_norm)) = 0;
x_pcg2D = reshape(x_pcg_norm, imageResolution);

MSE_pcg = mean((x_pcg_norm - v_true_vec_norm).^2);
...
if M > 0 && N > 0 && ~isempty(A_matrix) && ~isempty(b_vector)
    try
        if M < N
            fprintf('    Pinv: M<N method.\n');
            AA_inv = pinv(A_matrix * At_matrix, tolPinv);
            x_pinv_img_vec = At_matrix * (AA_inv * b_vector(:));
            pinv_method_desc = 'At*pinv(A*At)*b';
        else
            fprintf('    Pinv: M>=N method.\n');
            A_pinv = pinv(A_matrix, tolPinv);
            x_pinv_img_vec = A_pinv * b_vector(:);
...
x_pinv_proc = real(x_pinv_img_vec);
min_v = min(x_pinv_proc(:));
max_v = max(x_pinv_proc(:));

if (max_v - min_v) > eps
    x_pinv_norm = (x_pinv_proc - min_v) / (max_v - min_v);
else
    x_pinv_norm = zeros(N, 1);
end
x_pinv_norm(isnan(x_pinv_norm)) = 0;

x_pinv2D = reshape(x_pinv_norm, imageResolution);
MSE_pinv = mean((x_pinv_norm - v_true_vec_norm).^2);
...
residuals_admm_iters = zeros([numItersADMM 1]);
...
for k_admm = 1:numItersADMM
    figure(8); % Ensure Figure 8 is current
    v_upd = z_admm_grad_iter - u_admm_dual_iter;
    bb_upd = Atfun_admm_img(b_admm_vec) + rho_admm * opDtx_tv(v_upd);
    
    [x_vec_new, ~, ~, ~] = pcg(Hfun_pcg_admm, bb_upd(:), noise_sigma_admm, 25, [], [], x_admm_img_iter(:));
    ...
    if (max_x - min_x) > eps
        x_scl = (x_scl - min_x) / (max_x - min_x);
    else
        x_scl = zeros(size(x_scl));
    end
    
    MSE_curr = mean((x_scl - v_true_vec_norm).^2);
    PSNR_admm_iters(k_admm) = 10 * log10(1 / MSE_curr);
    r1 = b_admm_vec - Afun_admm(x_admm_img_iter);
    r2 = opDx_tv(x_admm_img_iter);
    tv_n = sum(sqrt(sum(r2.^2, 2)));
    residuals_admm_iters(k_admm) = 0.5 * sum(r1(:).^2) + lambda_tv_reg * tv_n;
    
    % Plotting
    subplot(2, 3, [1 4]);
    imagesc(x_coords_img * 1e3, z_coords_img * 1e3, I_true_matrix);
    axis image; colormap(gca, gray); colorbar; set(gca, 'YDir', 'normal');
    title('Target'); xlabel('x(mm)'); ylabel('z(mm)');
    
    subplot(2, 3, [2 5]);
    imagesc(x_coords_img * 1e3, z_coords_img * 1e3, reshape(x_scl, imageResolution));
    axis image; colormap(gca, gray); colorbar; set(gca, 'YDir', 'normal');
    title(sprintf('\\lambda=%.1e,\\rho=%.1f\nPSNR=%.2fdB,It %d', lambda_tv_reg, rho_admm, PSNR_admm_iters(k_admm), k_admm));
    xlabel('x(mm)');
...
fprintf('  ADMM-TV: Final PSNR=%.2f dB, Time=%.2fs\n', PSNR_admm_iters(end), runtime_ADMM_total);
